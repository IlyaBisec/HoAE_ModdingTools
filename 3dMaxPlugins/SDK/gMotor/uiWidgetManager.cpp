...
WidgetManager        g_WMgr;

//  global interface pointer
IWidgetManager* IWM = &g_WMgr;

IMPLEMENT_CLASS( WidgetManager );
...
void WidgetManager::OnDrop( int mX, int mY, DWORD ctx, DWORD obj )
{
    //  FIXME
...
} // WidgetManager::OnDrop

int    WidgetManager::GetFontID( const char* name )
{
    int nF = m_NFonts;
    for (int i = 0; i < nF; i++)
    {
    ...
    return -1;
} // WidgetManager::GetFontID

void WidgetManager::DestroyFont( int fontID )
{
}

void WidgetManager::CreateFullFontName( char* fullName, const char* fontName, int charW, int charH )
{
    sprintf( fullName, "%s%d_%d", fontName, charW, charH );
    _strlwr( fullName );
} // WidgetManager::CreateFullFontName

void WidgetManager::CreateFullFontName( char* fullName, const char* fontName, int height, DWORD charset, bool bBold, bool bItalic )
{
...
int    WidgetManager::CreateFont( const char* name )
{
    ...
    m_NFonts++;
    return m_NFonts - 1;
} // WidgetManager::CreateFont

int    WidgetManager::CreateFont( const char* name, int height, DWORD charset, bool bBold, bool bItalic )
{
    char fullName[256];
...
int    WidgetManager::CreateUniformFont( const char* texName, int charW, int charH )
{
    char fullName[256];
    ...
    pFont->m_TexHeight = texH;
    return id;
} // WidgetManager::CreateUniformFont

int    WidgetManager::GetStringWidth( int fontID, const char* str, int spacing )
{
    ...
    return w; 
} // WidgetManager::GetStringWidth

bool WidgetManager::DrawStringW( int fontID, const char* str, const Vector3D& pos, DWORD color, int spacing )
{
    Vector4D wpos( pos );
...
} // WidgetManager::DrawStringW

bool WidgetManager::DrawString( int fontID, const char* str, const Vector3D& pos, DWORD color, int spacing )
{
    ...
    return true; 
} // WidgetManager::DrawString

bool WidgetManager::DrawChar( int fontID, const Vector3D& pos, BYTE ch, DWORD color )
{
    if (fontID < 0 || fontID >= m_NFonts) return false;
    BitmapFont* pFont = m_Fonts[fontID];
    pFont->AddCharInst( ch, pos, color );
    return true; 
} // WidgetManager::DrawChar 

bool WidgetManager::DrawChar( int fontID, const Vector3D& pos, const Rct& uv, DWORD color )
{
    if (fontID < 0 || fontID >= m_NFonts) return false;
    BitmapFont* pFont = m_Fonts[fontID];
...
bool WidgetManager::DrawChar( int fontID, const Vector3D& pos, const Rct& uv, float w, float h, DWORD color )
{
    if (fontID < 0 || fontID >= m_NFonts) return false;
    BitmapFont* pFont = m_Fonts[fontID];
...
int    WidgetManager::GetCharWidth( int fontID, BYTE ch )
{
    if (fontID < 0 || fontID >= m_NFonts) return 0;
    BitmapFont* pFont = m_Fonts[fontID];
    return pFont->GetCharW( ch );
} // WidgetManager::GetCharWidth

int    WidgetManager::GetCharHeight( int fontID, BYTE ch )
{
    if (fontID < 0 || fontID >= m_NFonts) return 0;
    BitmapFont* pFont = m_Fonts[fontID];
    return pFont->GetCharH( ch );
} // WidgetManager::GetCharHeight

void WidgetManager::FlushText( int fontID )
{
    ...
    BitmapFont* pFont = m_Fonts[fontID];
    pFont->Flush();
} // WidgetManager::FlushText

void WidgetManager::OnDestroyRS()
{
    for (int i = 0; i < m_NFonts; i++)
    {
