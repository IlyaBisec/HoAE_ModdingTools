...
// Primitive::Swap
//-----------------------------------------------------------------------------
void Primitive::Swap(Primitive *pSrc) {
	swap(_priType, pSrc->_priType);
	swap(_vertFormat, pSrc->_vertFormat);
	swap(flags, pSrc->flags);
	swap(ibPos, pSrc->ibPos);
	swap(vbPos, pSrc->vbPos);
	swap(indNum, pSrc->indNum);
	swap(maxIndNum, pSrc->maxIndNum);
	swap(vertNum, pSrc->vertNum);
	swap(maxVertNum, pSrc->maxVertNum);
	swap(numPri, pSrc->numPri);
	swap(shaderID, pSrc->shaderID);
	swap(texID[0], pSrc->texID[0]);
	swap(texID[1], pSrc->texID[1]);
	swap(texID[2], pSrc->texID[2]);
	swap(UseMeshConst,pSrc->UseMeshConst);
	swap(MeshConst,pSrc->MeshConst);
	swap(ConstIdx,pSrc->ConstIdx);
                            ...
                            Vector3D& lb )
{
    ...
    for (int i = 0; i <= hSeg; i++)
    {
        ...
        for (int j = 0; j <= wSeg; j++)
        {
            ...
            vert[cV].x = vCur.x+lb.x;
            vert[cV].y = vCur.y+lb.y;
            vert[cV].z = vCur.z+lb.z;
...
DIALOGS_API void CreateEdgeTearMesh( const BaseMesh& pri, BaseMesh& edgeTear, DWORD clr )
{
    EdgeHash eHash;
...
DIALOGS_API void DrawBM( BaseMesh& bm )
{
...
void *memcpy_sse( void *dest, const void *src, size_t nbytes );

DIALOGS_API void DrawPrimBM( BaseMesh& bm )
{
    ...
    if (!pVert) return;
    //memcpy_sse( pVert, bm.getVertexData(), nV*bm.getVertexStride() );
    SSE_memcpy( pVert, bm.getVertexData(), nV*bm.getVertexStride() );
	//void *mmx_memcpy(void *dest, const void *src, size_t count );
	//mmx_memcpy( pVert, bm.getVertexData(), nV*bm.getVertexStride() );
...
{
    int nPasses = IRS->GetNShaderPasses( IRS->GetShader() );
    PrimitiveType priType = bm.getPriType();
    if (bm.isQuadList()) priType = ptQuadList;
...
{
    if( (DWORD(dest)&0xF) != 0 || (DWORD(src)&0xF) != 0 )
    {
