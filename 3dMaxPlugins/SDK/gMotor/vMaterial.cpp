...
/*****************************************************************************/
/*  ModelShader implementation  
/*****************************************************************************/
IMPLEMENT_CLASS(ModelShader);
IMPLEMENT_CLASS(ShaderTable);
...
void ModelShader::Expose( PropertyMap& pm )
{
    pm.start( "ModelShader", this );
    ...
    if (m_ShaderID == -1 && IRS) m_ShaderID = IRS->GetShaderID( GetScriptName() );
...
void ModelShader::SetScriptName( const char* name ) 
{ 
    m_ScriptName = name; 
    m_ShaderID = IRS->GetShaderID ( m_ScriptName.c_str() );
} // ModelShader::SetScriptName

void ModelShader::SetTex( int stage, const char* name )
{
    m_Texture[stage] = name;
    m_TexID[stage] = IRS->GetTextureID( name );
} // ModelShader::SetTex

void ModelShader::Validate()
{
...
bool ModelShader::operator ==( const ModelShader& sh ) const
{
    ...
    return true;
} // ModelShader::operator

void ModelShader::Render( int pass )
{
