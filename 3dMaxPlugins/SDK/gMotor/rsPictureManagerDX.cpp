...
protected:
	DXSurface*			m_pSurface;		//  as loaded from file
...
public:
	...
	bool		IsLoaded		() const { return m_pSurface != NULL; }
...
const int			m_MaxPicturesNum	= 256;
/*****************************************************************/
...
{
	PictureInstance				m_Pic[m_MaxPicturesNum];
	int							m_NPic;
...
public:
	...
	virtual int					GetImageID		( const BYTE* buffer, int size );
...
void PictureInstance::Load()
{
	...
	assert( pDevice );
	DX_CHK( pDevice->CreateImageSurface( m_Width, m_Height, 
											ColorFormatG2DX( m_ColorFormat ), &m_pSurface ) );
	HRESULT hRes = D3DXLoadSurfaceFromFile( m_pSurface, NULL, NULL, m_FileName, NULL, D3DX_FILTER_NONE, 0, &info );
	if (hRes != S_OK)
	{
...
void PictureInstance::CreateChunks()
{
	...
	while (cy < m_Height)
	{
		...
		while (cx < m_Width)
		{
			...
			chunk.m_TexID		= IRS->CreateTexture( texName, c_ChunkSide, c_ChunkSide, 
                                                        c_DrawFormat, 1, tmpManaged );
			...
			DXSurface* pChunkSurface = D3DRenderSystem::instance().GetTexSurface( chunk.m_TexID ); 
			RECT srcRect;
			srcRect.left		= cx;
			srcRect.top			= cy;
			srcRect.right		= right;
...
} // PictureInstance::GetDrawnSurface
...
{
	m_Primitive.create	( 4, 0, vfVertexTnL, ptTriangleList );
	m_Primitive.setNVert( 4 );
	m_Primitive.setNPri	( 2 );
	m_Primitive.setIsQuadList( true );
...
int	PictureManager::GetImageID( const char* fileName )
{
	...
	{
		strcpy( fullPath, GetRootDirectory() );
		strcat( fullPath, fileName );
	}

	for (int i = 0; i < m_NPic; i++)
	{
		if (m_Pic[i].HasFileName( fileName )) { LoadImage( i ); return i; }
	}
	
	D3DXIMAGE_INFO info; 
	HRESULT hRes = D3DXGetImageInfoFromFile( fullPath, &info );
	if (hRes != S_OK)
	{
		Log.Error( "Could not load image file %s.", fullPath );
		return -1;
	}

	int id = m_NPic;
	if (m_NPic == m_MaxPicturesNum)
	{
		Log.Error( "No more picture slots" );
		return -1;
	}
	PictureInstance& pic = m_Pic[m_NPic++];
	strcpy( pic.m_FileName, fullPath );
	...
	pic.m_bValid		= true;
	pic.m_ColorFormat	= ColorFormatDX2G( info.Format );
	return id;
} // PictureManager::GetImageID

int	PictureManager::GetImageID( const BYTE* buffer, int size )
{
	return -1;
}
...
void PictureManager::OnDestroyRenderSystem()
{
	for (int i = 0; i < m_NPic; i++) m_Pic[i].Unload();
}

void PictureManager::OnCreateRenderSystem()
{
}
