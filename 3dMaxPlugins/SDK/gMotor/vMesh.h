...
class DIALOGS_API BaseMesh : public Primitive{};
...
public:
    ...
    void            setTexture( int tex, int stage = 0 );
    void            setNVert( int n );
    void            setNPri( int n ); 
    void            setNInd( int n );
    void            create( int numVert, 
                            int numInd, 
                            VertexFormat vformat = vfVertex2t);
	void			ToMesh(BaseMesh& bm) const;
	void			FromMesh(BaseMesh& bm);
	void  reset();
    void  unlock();
    void  render();
};
...
DIALOGS_API void CreateEdgeTearMesh( const BaseMesh& pri, BaseMesh& edgeTear, DWORD clr = 0xFFFF0000 );
