...
class Box2: public RECT, public MaxHeapOperators {
	...
	corners). */
	IPoint2 GetCenter() const  { return IPoint2((left+right)/2, (top+bottom)/2); }
	/*! \remarks Returns the minimum x coordinate of the box. */
	int x() const { return ((left < right) ? left : right); }
	/*! \remarks Returns the minimum y coordinate. */
	int y() const { return ((top < bottom) ? top : bottom); }
	/*! \remarks Returns the width of the box. */
	int w() const { return abs(right-left)+1; }
	/*! \remarks Returns the height of the box. */
	int h() const { return abs(bottom-top)+1; }
	...
	*/
	...
	/*! \remarks Inequality operator. Determines whether b is different from Box2.
	Returns true iff the boxes are different. */
    bool operator!=( const Box2& b ) const { 	return !(operator==(b)); }
