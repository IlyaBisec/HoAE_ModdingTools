...
//! \brief Signature of the AppDataLoadCallback callback function that can be registered via Animatable::RegisterAppDataLoadCallback
/*! When an AppDataChunk with a specified SuperClassID and Class_ID is read from the scene file, the
specified callback proc is called. This is to permit versioning of the AppDataChunk.  
The values passed through the procs argument list are the Animatable holding the AppData, the Class_ID
and SClass_ID of the owner of the AppDataChunk, the ILoad*, and a Tab containing the sub-chunk ids.
*/
...
class Animatable : public InterfaceServer, public MaxSDK::Util::Noncopyable
{
	...
	private:
		...
		DWORD aflag_ex;
		CoreExport static void CallAppDataLoadCallbacks(Animatable* anim, ILoad* iload);
...
/// @{
		//! \brief Retrieves the (localizable) name of the plugin class
		/*! This name is usually used internally for debugging purposes. 
		    For Material plug-ins this method is used to put up the material "type" name in the Material Editor.
		...
		*/
		...
		//! Returns the name of the plugin class. \see void GetClassName(MSTR& s)
        MSTR ClassName() const { MSTR s; const_cast<Animatable*>(this)->GetClassName(s); return s;}        
		...
		*/
		...
		\sa  Class ClassID, class ID definititions in plugapi.h.
		*/
		CoreExport virtual Class_ID ClassID();
/// @}

		//! \brief Debug method to determine whether an object has been deleted
		/*! This method checks whether an object is alive in memory, or is
		deleted and is a dangling pointer.  The check is not perfectly accurate.
		A result of TRUE is guaranteed to be accurate - the object is deleted -
		however a result of FALSE may be inaccurate.  This method should be used
		together with other tools to aid in debugging and bulletproofing.
		\param anim The Animatable object
		\return Returns TRUE if the object is invalid, FALSE if state is uncertain
		*/
		CoreExport static BOOL IsDeleted( Animatable* anim );
...
//! \name AppData, Interfaces, and Properties
/// @{
		...
		<b>{ return 0; }</b>
		\par Sample Code:
		This code is from <b>/MAXSDK/SAMPLES/CONTROLLERS/path_cnstrnt.cpp</b>. It
		...
		\snippet samples\controllers\path_cnstrnt.cpp Animatable.SetProperty Example
		  */
		...
		\snippet samples\controllers\path_cnstrnt.cpp Animatable.GetProperty Example
		*/
		CoreExport virtual void *GetProperty(ULONG id);

		/** A function to directly add arbitrary properties to this object
			developers should ensure that the properties ID does not conflict
			with any Max-specific IDs.
			\param prop The new property to add */
		CoreExport void AppendProperty(AnimProperty* prop);

		// Find a property.  I have no idea why GetProperty is implemented as return NULL.  Seems idiotic to me.
		// TODO: Tekken or higher - merge this function with the one above
		/** Find any property.  This allows developers to add arbitrary properties to Animatables.
...
/// \name Methods dealing with Keys and Track View
/// @{
...
/// @}
		...
		functions).
		\param subAnim - The ID of the SubAnim to check.
		...
		// correspondence. This is used for copying and pasting in the track
		// view. If a client does not wish an anim to be copy/paste-able then
		// it can return -1 even if there is a corresponding ref num.
...
/// \name Enumeration of Anims and Auxiliary Files
/// @{
...
//! \name Methods called when rendering is started and finished
/// @{
...
/// \name Operations to a selected block of time
/// @{
		...
		<b>{}</b>
		\par Sample Code:
		\snippet samples\objects\morphcnt.cpp Animatable.ReverseTime Example
		*/
		...
		<b>{}</b>
		\par Sample Code:
		\snippet samples\objects\morphcnt.cpp Animatable.ScaleTime Example
		*/
		...
		<b>{}</b> 
		\par Sample Code:
		\snippet samples\objects\morphcnt.cpp Animatable.InsertTime Example
		*/
		...
		\par Sample Code:
		\snippet samples\objects\morphcnt.cpp Animatable.MapKeys Example
		*/
		...
		<b>{}</b>
		\par Sample Code:
		\snippet samples\objects\morphcnt.cpp Animatable.FlagKey Example
		*/
		...
		<b>{return -1;}</b>
		\par Sample Code:
		\snippet samples\objects\morphcnt.cpp Animatable.GetFlagKeyIndex Example
		*/
		...
		<b>{}</b>
		\par Sample Code:
		\snippet samples\controllers\boolcntrl\boolcntrl.cpp Animatable.MoveKeys Example
		*/
		...
		\par Sample Code:
		\snippet samples\controllers\boolcntrl\boolcntrl.cpp Animatable.SetSelKeyCoordsExpr Example
		*/
		...
		\param flags One of the following values:
		* ADJTAN_LOCK - Indicates the tangents are locked.
		* ADJTAN_BREAK - Indicates the tangents have been broken.
		*/
		...
		adjusts them. If a plug-in doesn't have tangent handles, this method
		may be ignored. This method is called if the user selects one of the
		handles in the viewport. This method is passed the change in angle and
		length of the handle the user wishes to perform.
		...
		\param hit This identifies the handle that was selected.
		\param dim The parameter dimension. See Class ParamDimensionBase.
		\param angle The change in angle the user is requesting (in radians).
		\param length The change in length the user is requesting.
		\param flags One of the following values:
		* ADJTAN_LOCK - Indicates the tangents are locked.
		* ADJTAN_BREAK - Indicates the tangents have been broken.
		*/
...
/// @}
		...
		\param t - The time of the SubAnim to commit.
		\par Default Implementation:
		\code
		{
...
/// \name Drawing and hit testing tracks
/// @{
...
/// \name Drawing and hit testing function curves
/// @{
		...
		virtual void EditTrackParams(
			...
			ParamDimensionBase *dim,
			const MCHAR *pname, // The name of the parameter as given by the client
			HWND hParent,
			IObjParam *ip,
			DWORD flags) {}
...
/// \name Methods dealing with Note Tracks
/// @{
		/*! \remarks Implemented by the System.\n\n
...
/// @{
		...
		\par Sample Code:
		\snippet samples\hardwareshaders\MetalBump9\MetalBump.cpp Animatable.EnumAuxFiles Example
		*/
		CoreExport virtual void EnumAuxFiles(AssetEnumCallback& assetEnum, DWORD flags = FILE_ENUM_ALL);
/// @}

//! \name Bitmap Related Methods
//! @{
...
/// \name System Plug-In Related Methods
/// @{
...
/** \name AppData, Interfaces, and Properties
 *  The following methods deal with AppData. This is application specific data that may be
	attached to any Animatable in the scene. With these APIs any 3ds Max object
	(controller, object, node, modifier, material, etc.) can have custom data attached by other objects.
	These chunks are saved in the .MAX file and can be accessed through the object they are attached to.
*/
//! @{
...
//! @}
		...
		//! \brief Registers a callback proc that is called when an AppDataChunk is read from a scene file.
		/*! When an AppDataChunk with the specified SuperClassID and Class_ID is read from the scene file, the
		specified callback proc is called. This is to permit versioning of the AppDataChunk.  
		Duplicate callbacks (calls to this method with identical parameters) will not be registered.
		The values passed through the procs argument list are the Animatable holding the AppData, the Class_ID
		and SClass_ID of the owner of the AppDataChunk, the ILoad*, and a Tab containing the sub-chunk ids.
		\code
		...
		\param cid - The Class_ID of the owner of the chunk.
		\param sid - The SuperClassID of the owner of the chunk.
		\param proc - The callback proc to be called.
		\return true if the callback was registered.
		*/
		CoreExport static bool RegisterAppDataLoadCallback(const Class_ID& cid, SClass_ID sid, APPDATALOADPROC proc);

		//! \brief Unregisters a callback proc that is called when an AppDataChunk is read from a scene file.
		/*! Unregisters the specified callback proc is called. Callback procs do not need to be unregistered 
		prior to 3ds Max shutdown.
		\param cid - The Class_ID of the owner of the chunk.
		\param sid - The SuperClassID of the owner of the chunk.
		\param proc - The callback proc to be called.
		\return true if the callback was unregistered.
		*/
		CoreExport static bool UnRegisterAppDataLoadCallback(const Class_ID& cid, SClass_ID sid, APPDATALOADPROC proc);

		//! \brief Registers a callback proc that is called when an AppDataChunk is read from a scene file.
		/*! When an AppDataChunk with the specified sub-chunk id is read from the scene file, the
		specified callback proc is called. This is to permit versioning of the AppDataChunk.  
		Duplicate callbacks (calls to this method with identical parameters) will not be registered.
		The values passed through the procs argument list are the Animatable holding the AppData, the Class_ID
		and SClass_ID of the owner of the AppDataChunk, the ILoad*, and a Tab containing the sub-chunk ids.
		...
		\endcode
		\param sbid - The extra ID that lets the owner identify its sub-chunks.
		\param proc - The callback proc to be called.
		\return true if the callback was registered.
		*/
		CoreExport static bool RegisterAppDataLoadCallback(DWORD sbid, APPDATALOADPROC proc);
		...
		\return true if the callback was unregistered.
		*/
		CoreExport static bool UnRegisterAppDataLoadCallback(DWORD sbid, APPDATALOADPROC proc);


//! \name Interactive Adjustment
/// @{
...
/// @{
		...
		*/
		...
		developer chooses).
		\param flags See \ref Flags_for_AddAnimatable___and_SvTravereseAnimGraph__.
		...
		\param gom Points to the schematic view window manager.
		\param gNode Points to this node in the schematic view.
		\return  See <a href="https://msdn.microsoft.com/en-us/library/dd183449(v=vs.85).aspx">COLORREF-DWORD format</a>.
		\par Default Implementation:
...
/// @}
		...
		//! \brief  This method is called to access the object specified name to use for the Maxscript macrorecorder.
		/*! The Maxscript macrorecorder will determine the name of the object to use when emitting code. In some cases
		it cannot determine a name for the object, for example when setting a value on a ParamBlock when the ParamBlock
		owner is a utility plugin singleton. This method allows the object to specify the name to be used.  
		If this method returns true and objectSpecifiedName is empty, no code will be emitted. This is useful in the case
		where you don't changes to an object's parameters to be macro recorded.
		\param used_as_rhs_value [in] The object is being assigned to a property, return constructor expression (i.e., "box()").
		\param objectSpecifiedName [out] The object name for Maxscript macrorecorder to use.
		\return  Returns true if a name is specified; otherwise false. If true and objectSpecifiedName is empty, no code will be emitted.*/
		virtual bool GetMacroRecorderName(bool used_as_rhs_value, MSTR &objectSpecifiedName) { return false; }
};
#pragma warning(pop)
