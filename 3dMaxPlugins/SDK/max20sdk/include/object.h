...
#define HIT_MANIP_SUBHIT        (1<<8)
 ...
 /** This flags tells the hittesting that the mesh has been a mirroredtm/negative scale and the winding order needs to be flipped 
	 to compute the facing direction
 */
...
//! \name Object Level Display, Hit Test, Snap, and Bounding Box Methods
/// @{
	...
	\return  Default return value is -1, means this object doesn't have display function. 
	0 means it didn't draw anything in current frame. Otherwise return positive values.. */
...
//! \name Object Name and Create Callback
/// @{
...
/// @{
	...
	Class IParamArray. */
	virtual IParamArray *GetParamBlock() {return nullptr;}
...
//! \name Sub-Object Selection - Affine transformation methods
/// @{
...
/** \name Sub-Object Selection Methods
...
///@{
	...
	\return  Default return value is -1, means this object doesn't have display function. 
	0 means it didn't draw anything in current frame. Otherwise return positive values.. */
	virtual int Display(TimeValue t, INode* inode, ViewExp *vpt, int flags, ModContext* mc) { return -1; };   // quick render in viewport, using current TM.         
	 ...
	 * support named sub object selection sets. Methods in the
	 * interface passed to objects allow them to add items to the
	 * sub-object selection set drop down list.
	 * A modifier that wishes to support this capability maintains
	 * its list of named sub-object selections. When a user enters
	 * the sub-object selection mode, the modifier adds its named selection
	 * sets into the drop down (using Interface::AppendSubObjectNamedSelSet()).
...
///@}
	...
	/** \name Sub-Object Centers and TMs
...
//! \name UVW Methods
/// @{
	/**	Call this method to find out if the object has UVW coordinates. This method
	...
	/**	Call this method to find out if the object is has UVW coordinates for the
...
//! \name Object Name, Properties, Display, and IntersectRay Methods
//! @{	
...
//! @}
	...
	*/
	...
	Class Class_ID, Class IDs defined in plugapi.h.
	...
	Class IDs defined in plugapi.h. */
	...
	// The derived class must incorporate its base class' validity into the resulting interval.
	...
	\return  The validity interval of the object, including that queried from the base class. */
...
//@{
#define CAM_HITHER_CLIP         1	//!< The hither/near distance
#define CAM_YON_CLIP            2	//!< The yon/far distance.
//@}
...
/** \defgroup Returne_valuse_of_ParticleCenter___ Return values of ParticleCenter() 
...
class ShapeObject : public GeomObject
{
	...
	int cacheType;
protected:
	CoreExport virtual bool IsRequiresLegacyDisplayMode() const;
public:
	...
	*/
	...
	//! \deprecated This function deprecated in Max 2018. Please use ShapeObject::NumberOfCurves(TimeValue t).
	/** Returns the number of polygons in the shape. */
	MAX_DEPRECATED virtual int NumberOfCurves() { DbgAssert(0); return 0; }                 // Number of curve polygons in the shape
	/** Returns the number of polygons in the shape at the given time
	\param t The time to check.
	\return  the number of polygons in the shape. */
	virtual int NumberOfCurves(TimeValue t)=0;	// Number of curve polygons in the shape at the given time
	...
	/** Implemented by the System.
	This method handles the saving of references when saving to previous
	version files.  The ShapeObject maintains a parameter block for the
	renderable shape parameters and this needs to be adjusted when saving to
	older versions of 3ds max.
	\param referenceSaveManager The ReferenceSaveManager to use. See Class ReferenceSaveManager. */
...
public:
	...
	*prev )\n\n
	...
	\param objParam The interface pointer passed to the plug-in.
	...
	*/
	...
	// Get/Set the twist correction flag.  When true, closed renderable
	// shapes will correct any twisting due to complex bends.
	...
	// Get/Set the main capping flag.  When true, the renderable version
	// of the shape will have the ends of open curves capped
	/** Queries the main capping switch. */
	CoreExport BOOL GetCap();
	/** Sets the main capping switch. */
	CoreExport void SetCap(BOOL flag);

	// Get/Set the quad capping flag.  When true, and if the main capping
	// flag is set, the renderable version of the shape will have the ends
	// of open curves capped with quadrilaterals.
	...
	/** Get/Set the cap segments count. This value is used for quad capping
	and allowable ranges are from 1 to 200 */
	/** Queries the cap segments value at the specified time.
	\param t The time at which to obtain the segment count.
	\param ivalid The validity interval. */
	CoreExport int GetCapSegments(TimeValue t, Interval &ivalid);
	/** Sets the cap segments value for the specified time.
	\param t The time at which to set the segment count.
	\param count The count you wish to set. */
	CoreExport void SetCapSegments(TimeValue t, int count);

	/** Get/Set the sphere capping value used in quad capping.
	Range is 0 (flat cap) to 1 (spherical). */
	/** Queries the sphere cap value.
	\param t The time at which to obtain the segment count.
	\param ivalid The validity interval. */
	CoreExport float GetSphereCap(TimeValue t, Interval &ivalid);
	/** Sets the sphere capping value (0-1).
	\param t The time at which to set the sphere amount.
...
//! \name InputType, ChannelsUsed, and ChannelsChanged
//! @{
	...
	/**	If a modifier want to make it possible to switch dynamically between
...
//! \name Notification of Input Changed
//! @{
	...
	method will cause an endless loop. Simply putting a guard in, that checks
...
//! @}
	...
	/** To determine the input type of a modifier, an instance of the modifier needs to 
	be created. If the input type for a large number of modifiers are gathered (for example,
	when opening the modifier panel modifier dropdown list), creating an instance
	of each modifier is slow. This method creates an instance of the modifier if needed,
	and caches the input type. Later requests are satisfied via the cached value.
	\param cd [in] The classdesc of the modifier. Valid superclasses are OSM_CLASS_ID and WSM_CLASS_ID.
	\param inputType [out] The input type of the modifier. See InputType() for a list of
	the valid values.
	\return  true if a modifier class with the specified SClass_ID and Class_ID was 
	found, and the output inputType was set to a valid value.
	*/
	CoreExport static bool GetInputType(ClassDesc* cd, Class_ID &inputType);
