...
public:
	...
	EP_VertWeldMouseProc(PatchObject* mod, IObjParam *i) : toVert(0), fromVert(0) {po=mod;mpIP=i;}
	BOOL HitTest( ViewExp *vpt, IPoint2 *p, int type, int flags, int subType );
	BOOL HitAVert(ViewExp *vpt, IPoint2 *p, int& vert);
...
/*! \sa  Class GeomObject, Class IPatchOps, Class IPatchSelect, Class IPatchSelectData, Class ISubMtlAPI, Class AttachMatDlgUser, Class PatchMesh, Class Patch,  Class Mesh,  Class TessApprox, ~{ Patches and Faces }~.\n\n
...
class PatchObject: public GeomObject, IPatchOps, IPatchSelect, IPatchSelectData, ISubMtlAPI, AttachMatDlgUser {
	...
	MaxSDK::Graphics::IMeshDisplay2* mpMeshDisplay;
	public:
...
// 7/20/00 TH -- Relax controls
		...
		CoreExport virtual const MCHAR *GetObjectName();
...
public:
		...
		*/
		...
		CoreExport virtual void BeginEditParams( IObjParam *objParam, ULONG flags,Animatable *prev);
		CoreExport virtual void EndEditParams( IObjParam *objParam, ULONG flags,Animatable *next);
...
class POModRecord: public MaxHeapOperators {
	public:
		...
		virtual ChannelMask Parts()=0;				// Flags of parts (channels) modified by this
	};
...
class SingleRefMakerPatchNode : public SingleRefMaker{
	public:
		...
		virtual void GetClassName(MSTR& s) { s = _M("SingleRefMakerPatchNode"); } // from Animatable
	};
...
class SingleRefMakerPatchMtl : public SingleRefMaker{
	public:	
		...
		RefResult NotifyRefChanged( const Interval& changeInt,RefTargetHandle hTarget, 
		PartID& partID, RefMessage message, BOOL propagate );
		virtual void GetClassName(MSTR& s) { s = _M("SingleRefMakerPatchMtl"); } // from Animatable
};
