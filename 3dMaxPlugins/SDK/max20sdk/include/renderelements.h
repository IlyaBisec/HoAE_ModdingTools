...
/*! \sa  Class IRenderElement, Class ShadeContext, Class IllumParams , ~{ Render Elements }~\n\n
\par Description:
...
sub-class from here. All render elements that support the 3ds Max renderer should
derive from this class. The class implements a handler for the ShadeOutputIndex
of the elements value in the ShadeOutput array. Access to the element's value is
...
public:
	...
	the one requested. This method must be implemented, though some elements
	merely stub it out or clear the output element value. It should put it's
	output in <b>sc.out.elementVals[ShadeOutputIndex() ]</b>. Even if you do
	...
	// should this element be visible as a choice when using the Scanline Renderer?
	/*! \remarks Specifies whether this render element is compatible with the Default 
	Scanline Renderer and should show up in the Render Element Dialog for the Scanline
	Renderer.\n\n
	Several render elements derive from MaxRenderElement class
		...
		Therefore, this method was added, and it returns TRUE if the element is actually 
		meaningful in Scanline.
		By returning FALSE here, you can avoid having your render element appear in
	...
	*/
	...
	existing class it would invalidate the plug-ins that used the class. 
	This method allows additional functionality to be added to the interface
...
public:
	...
	/*! \remarks Returns the index of the parameter based on the pName\n\n
	\code
	const int FindParamByName1( MCHAR* name ) MAX_SEALED
	{
		for( int i = 0; i < 8; ++i ) {
	...
	\endcode */
	virtual	const int FindParamByName( const MCHAR *pName ) = 0;
...
//! \deprecated in 3ds Max 2017. All methods of IRenderElementCompatible have been moved to class Renderer.
/*! See Renderer::CompatibleWithAnyRenderElement() and Renderer::CompatibleWithRenderElement() */
typedef MAX_DEPRECATED Renderer IRenderElementCompatible;
...
/*! \sa  Class IRenderElement, Class FPMixinInterface, Class ReferenceTarget, ~{ Render Elements }~\n\n
\par Description:
