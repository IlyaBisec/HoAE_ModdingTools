...
hemispheres smoothed separately (i.e. not smoothed across the equator), then
...
public:
	...
	/** Deletes the specified number of elements from the specified
...
#define MESH_DONTTRISTRIP (1<<15)	//!< \internal Mesh flag to determine whether to use tri-striping or not
...
#define MESH_UVW_INVALID    (1<<20) //!< \internal Mesh flag that means any MapChannel data is changed.
#define MESH_CLEARMAPCHANNEL (1<<21)    //!< Mesh Flag if set  new tv face data will be cleared otherwise it will use a default planar
#define MESH_VERTEXCOLOR_INVALID    (1<<22) //!< \internal Mesh flag that means any of the vertex color channel data is changed.
...
SubObjHitList use Tab instead of linked-list in Max 2017 for performance of large amount of data.
The constructor's change is to keep code compatibility. Although internal storage changed, client
code only needs recompile without any modification.
...
struct MeshSubHitRec {
	...
	*/
	...
	/** The second level index of the sub-object component. Usually it is used for MNMesh's diagonal
	selection, in this case the index is the poly face index while index2 is the diagonal index 
	within the poly face. 
	*/
	int		index2;

	/** Constructor. The data members are initialized to the data
	members passed.
	\param dist distance of the hit point
	\param index the sub object's index
	*/
	...
	struct Iterator {
		...
		bool operator == (const Iterator& ref) const { return (blockPtr == ref.blockPtr && localIndex == ref.localIndex); }
		bool operator != (const Iterator& ref) const { return !(*this == ref); }
		MeshSubHitRec* operator -> () { return &(*(*this)); }
	};
};
...
class SubObjHitList: public MaxHeapOperators {
	...
	public:
		...
		/** Returns the iterator of hit records. */
		DllExport MeshSubHitRec::Iterator begin();
		DllExport MeshSubHitRec::Iterator end();

		/** Allocates and adds a new hit record to the list.
		\param dist The distance to the hit. 
		\param index The index of the hit. 
		\param index2 The second level index of the sub-object component, for example 
		the diagonal selection in an MNMesh.  The default is -1, not used.  */
		DllExport void AddHit( DWORD dist, int index, int index2 = -1);

		/** Reserve memory to reduce allocation time.
		\param size size of the memory to reserve
		*/
		DllExport void Reserve(size_t size);

		/** Returns true if the hit list contains no record. */
...
/** The space accelerating structure used for fast space looking-up of sub-objects like faces, edges ,vertices.
Currently the implementation of this class is not exposed in SDK.
*/
class Mesh_SpaceAccelData;
class Mesh_SelParams;
...
class Mesh : public BaseInterfaceServer {
	...
	private:
		...
		*/
		...
		void 		Draw3DVisEdgeList(IHardwareShader *phs, DWORD vflags);
...
//! \name Texture Vertex-Texture Face Methods
//! @{		
...
//! \name Normals/Smoothing/Edges/Face check Functions
//! @{		
...
//! \name Render/Hit Test/Snap Methods
//! @{		
		...
		\param is This pointer to an InterfaceServer can be used to get hold of the
		IXTCAccess pointer. 
...
//! \name Intersect Ray/Weld/Optimize/Apply Mapping
//! @{			
		/** \internal This method is used internally. */
		...
		DllExport void Optimize(
			...
			float bias, float maxEdge, DWORD vflags, 
...
//! @{			
		...
		// by the face deletion. This set can then be passed to
...
//! @{		
		...
		*/
		...
		If TRUE, the edge visibility is ignored but the threshold is still
		relevant. 
		\param af 
      ...
      */
      ...
      long *      vFlags;
		...
		*/
		...
		\param ma the material array assigned to the mesh \n
...
public:
		...
		#pragma region Space Accelerating Data Structure
		/// \name Space Accelerating Data Structure
		/// These functions and member variable handle the space accelerating data structure used internally by Max system.
		/// 3ds Max 2017 release introduced the space accelerating data structure and algorithm to speed up the methods related to space looking-up.
		///	The space accelerating data can significantly improve the space looking-up algorithm, however the system may take some time to build
		///	the space accelerating data.It is recommended to use the following functions in the scenario of updating once then looking up multiple times.
		//@{
private:
		// Pointer to space accelerating data maintained by Max system
		Mesh_SpaceAccelData* mpSpaceAccelData;
public:
		...
		*/ 
		...
		\remarks Invoking InvalidateGeomCache() or InvalidateTopologyCache() will dirty the space accelerating data.
		*/
		DllExport bool IsAccelDataDirty();

		/** Update the space accelerating data.
		\remarks This function should only be invoked when the plug-in needs to invoke following methods before 
		InvalidateGeomCache() or InvalidateTopologyCache() gets invoked.
		*/
		...
		DllExport BOOL SubObjectHitTest(GraphicsWindow *gw, Material *ma, HitRegion *hr,
			DWORD vflags, SubObjHitList& hitList, int numMat = 1);

		/** This function computes the render normal for the specified face and corner. Unlike the method of buildNormals() and 
		buildRenderNormals(), this function does not rely on whole RVertex array to achieve normal computation, which means it is 
		lightweight and it quite efficent to do the normal computation for a given face and corner index.
		The only limitation is that this function will update the space accelerating structure if it is needed.
		\param face_i
		The index to the face.
		\param corner
		The corner index within the face(must between 0 and 2 inclusively)
		*/
		...
		The index to the face.
		\param bary
		The input barycentric coordinates to compute the interpolated normal.
		*/
		...
		*/
		...
		// For internal use, do not invoke it unless you know what it does
...
Note: This method is still in the SDK, but it is now obsolete. Calls to
