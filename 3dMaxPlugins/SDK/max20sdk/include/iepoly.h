...
enum epolyParameters { ep_by_vertex, ep_ignore_backfacing, ep_show_normals,
	...
	ep_edge_chamfer_smooth,		// bool		- For Quad Chamfers, tells whether to smooth the output
	ep_edge_chamfer_smooth_type,		// int		- For Quad Chamfers, when epm_edge_chamfer_smooth==true, tells what kind of smoothing (see EP_QUAD_SMOOTH_XXX)
	ep_edge_chamfer_smooth_threshold,		// float		- For Quad Chamfers, when epm_edge_chamfer_smooth==true, gives smoothing threshold

	// New for 2016
	ep_hard_edge_display_color,	// color - Specifies color of hard (unsmoothed) edge display in viewports.
	ep_hard_edge_display,		// int - Controls display of hard (unsmoothed) edges.  See EP_HARD_DISPLAY_XXX)

	// New for 2018
	ep_edge_chamfer_quad_intersections,	// Edge intersection type for quad chamfer. zero = original, nonzero = quad
...
enum epolyButtonOp { epop_hide, epop_unhide, epop_ns_copy, epop_ns_paste,
	...
	//2016
	epop_make_hard_edges, epop_make_smooth_edges,
	epop_select_hard_edges, epop_select_smooth_edges
};
...
enum epolyCommandMode { epmode_create_vertex, epmode_create_edge,
	...
	//new command mode that lets the user pick the subobject mode
...
enum ePolyMethods { epfn_hide, epfn_unhide_all, epfn_named_selection_copy,
	...
	epfn_break_verts, epfn_divide_face, epfn_divide_face2, epfn_divide_edge, epfn_divide_edge2,
...
//! @{
	...
	Parts of the mesh that have been changed, such as <b>GEOM_CHANNEL</b>,
	<b>TOPO_CHANNEL</b>, etc.
	\par Default Implementation:
	<b>{ }</b> */
	...
	// you call LocalDataChanged (GEOM_CHANNEL).  Should also be done if the soft
	...
	is automatically done when you call <b>LocalDataChanged(GEOM_CHANNEL)</b>.
...
//! \name Selection and Component Flag Access
//! @{
...
/** \name UI Button and Command Mode Methods
 * These methods allow developers to simulate clicking the buttons available in the UI.
 * Some buttons in Editable Poly such as "Delete", complete an operation when clicked.
 * Others, such as "Create", enter into an interactive command mode when they are clicked.
 */
///@{
...
//! \name Component Data Access Methods
//! @{	
	...
	\ref Vertex_Data_Index_Options (which are defined in <b>mesh.h</b>).\n\n
	<b>int *numSel</b>\n\n
	...
	\ref per_edge_data "Edge data channel index values" (which are defined in <b>mnmesh.h</b>).\n\n
	<b>int *numSel</b>\n\n
	If non-NULL, this is filled in with the current number of flagged
	edges.\n\n
	<b>bool *uniform</b>\n\n
	...
	\par Parameters:
	<b>int channel</b>\n\n
	The vertex data. See the \ref Vertex_Data_Index_Options
	\par Default Implementation:
	<b>{ }</b> */
	...
	\par determined - The bool this points to (which should not be NULL) is filled with:
		FALSE if there are no selected faces or if selected faces have different
		material indices.
		TRUE if at least one face is selected and all selected faces have the same material ID. */
	virtual int GetMatIndex (bool *determined) { if (determined) { *determined = false; } return 0; }
...
//! \name Displacement Approximation Methods
//! @{	
...
protected:
	...
	*/
	...
	enum ManipulateGrips
	{
		...
		eVertexCrease,
		// The following is an end-of-enums semaphore -- Used to get the count of enums here.
		// Leave it as the last entry.
...
//! This new class extends EPoly with new functionality, for new hard/smooth edge workflow.
class EPoly18: public EPoly13
{
public:
	...
	\param flag - Edges marked with this flag (\ref MN_MESH_Flags) will be set to hard if possible
		by setting appropriate smoothing groups in adjacent faces.
	\return true if any faces had smooothing groups altered
		*/
	virtual bool		EpfnMakeHardEdges (DWORD flag)=0;
	/** Make the edges marked with the specified flag smooth
	\param flag - Edges marked with this flag (\ref MN_MESH_Flags) will be set to smooth if possible
		by setting appropriate smoothing groups in adjacent faces.
	\return true if any faces had smooothing groups altered
		*/
	virtual bool		EpfnMakeSmoothEdges (DWORD flag)=0;

	/** Select hard (unsmoothed) edges.  This operation selects edges between any faces which
		do not share any smoothing groups.
		*/
	virtual void		EpfnSelectHardEdges ()=0;
	/** Select smoothed edges.  This operation selects edges between any faces which
		share any smoothing groups.
		*/
