...
class ScaleValue: public MaxHeapOperators {
   ...
   Point3 s;
   Quat q;
   /*! \remarks Constructor. s and q have their own initializers.*/
   ScaleValue() { }
...
public:
   ...
   struct DelayedOp : public MaxHeapOperators {
	  ...
	  DelayedOp(int, int, float z)
		...
		{
			return code == b.code && arg == b.arg;
		}
   };

	struct OpQueue : public MaxSDK::Array<DelayedOp>
	{
		typedef MaxSDK::Array<DelayedOp> BaseClass;
		int head;
		OpQueue() : BaseClass(), head(0) {}
		void Clear() { removeAll(); head = 0; }
		int QCount() const { return (int)(BaseClass::length() - head); }
		DelayedOp& Shift() { return BaseClass::operator[](head++); }
		void Push(DelayedOp& op) { append(op); }
	};
...
\code
#define
GetEaseListInterface(anim) ((EaseCurveList*)anim-\>GetInterface(I_EASELIST))
\endcode
...
class EaseCurveList : public ReferenceTarget {
   ...
   public:
	  ...
	  */
	  ...
	  /*! \remarks Returns the 'i-th' ease curve.
	  \par Parameters:
	  <b>int i</b>\n\n
...
\code
#define
GetMultListInterface(anim) ((MultCurveList*)anim-\>GetInterface(I_MULTLIST))
\endcode
This may be used to access the methods of this class as follows:
		\code
...
class MultCurveList : public ReferenceTarget {
   ...
   public:
	  ...
	  /*! \remarks Destructor. All references are removed from this class. */
	  CoreExport ~MultCurveList();

	  /*! \remarks This method starts with a value of <b>1.0f</b>, multiplies
	  it by each enabled multiplier curve value in the list, and returns the
	  resulting value.
	  \par Parameters:
	  <b>TimeValue t</b>\n\n
	  The time at which to get the multiplier values.\n\n
	  <b>Interval \&valid</b>\n\n
	  The interval that is adjusted to reflect the validity of all the
	  multiplier curve controllers validity. */
	  ...
	  /*! \remarks Returns the 'i-th' multiplier curve.
	  \par Parameters:
	  <b>int i</b>\n\n
	  The index of the multiplier curve to return. */
...
\par Description:
An extension to <b>Class InitJointData2</b> to support various Spring
...
/*! \sa  Class ReferenceTarget, Class IKeyControl, Class IKDeriv, Class IKEnumCallback, Class StdControl, 
Class AngAxis, \ref Additional_Controller_Related_Functions, Class Quat, Class Interval, Class Matrix3, Class Point3, Class ScaleValue, Class JointParams, Class SetXFormPackt.\n\n
\par Description:
...
/** \name Undo/Redo Methods
 *  Controllers, like objects and modifiers, need to be able to undo and redo themselves.
 *  Whenever a controller is about to modify its data, it checks the state of the global
 *  Hold object to see if it is holding. If so it must register a RestoreObject with the hold
 *  (see the Advanced Topics section eUndo/Redo). Controllers also support another type of undo
 *  and redo through two methods: CommitValue() and RestoreValue().
 *  \n The purpose of this 'inner' hold and restore buffer is not to hold and restore the
 *  entire state of the controller, but to hold and restore the value of the controller at a
 *  single instant in time. When SetValue() is called on a controller with the commit parameter
 *  equal to zero, the controller records the new value, but does not necessarily modify any data.
 *  For example, a keyframe controller doesn't actually generate a new key ?it just updates a
 *  cache it maintains. Then, if the controller is asked to evaluate itself at the exact same
 *  TimeValue for which the controller was just set, it can just return the cached value.
 *  \n The RestoreValue() method will simply throw out the temporary value whereas the
 *  CommitValue() method will cause the value to be actually committed (a key generated
 *  in the case of a keyframe controller).
 *  \n The purpose of this inner hold and restore is for iterative procedures that need
 *  to set values many times at a single TimeValue and don't want to incur the overhead of
 *  things like recalculating the tangents at adjacent keys. Some examples of these types
 *  of procedures are inverse kinematics and collision detection. 
 */
///@{
...
//! \name Transform Controller Methods
//! @{
...
//! @{
	  ...
	  <b>{return FALSE;}</b>
	  \par Sample Code:
      This is the code used inside 3ds Max' reactor controller (in 
      MAXSDK/samples/controllers/reactor/reactor.cpp).       
	  A plug-in could provide an implementation for this method using a similar
	  concept.\n\n
      \snippet samples/controllers/reactor/reactor.cpp Control.ChangeParents Example
	   */
	  virtual BOOL ChangeParents(TimeValue t,const Matrix3& oldP,const Matrix3& newP,const Matrix3& tm) {return FALSE;}
//! @}

//! \name Get/Set Value Methods
//! @{
	  ...
	  ::GetSetMethod parameter <b>method</b>. 
	  See "val parameter Handling" below for a list of the possible cases, and how
	  the value should be stored in each case.	  
	  ...
	  \par <b>val</b> parameter Handling
	  \par 
	  float:
	  ...
	  Position:
	   ...
	   - method == CTRL_RELATIVE, *val points to a Matrix3. The controller should apply its value to the matrix by pre-multiplying its position. 
	     \code Matrix3 *mat = (Matrix3*)val; \endcode
	     Point3 v = the computed value of the controller...
	     \code mat->PreTranslate(v); \endcode		 
	  \par 
	  Rotation:
	   ...
	   - method == CTRL_RELATIVE, *val points to a Matrix3. The controller should apply its value to the matrix by pre-multiplying its rotation.
		 ...
		 \code PreRotateMatrix(*mat,q); \endcode
	  \par
	  Scale:
	   ...
	   \code ApplyScaling(*mat,s); \endcode
	  \par
	  Transform (Matrix3)
	   - method == CTRL_ABSOLUTE, *val points to a Matrix3. The controller should simply store the value.
	     <b>Note:</b> Developers should only pass CTRL_RELATIVE when getting the value of a Matrix3 controller. This is because the controller may use the matrix as input to compute the value. Therefore it is not acceptable to use CTRL_ABSOLUTE to get the value.
	   - method == CTRL_RELATIVE, *val points to a Matrix3. The controller should apply its value to the matrix by pre-multiplying. When GetValue() is called on a transform controller the method is CTRL_RELATIVE and the matrix passed is usually the parent of the node.
	   <b>Note:</b> Important for Matrix3 Controllers: when Control::SetValue() is called *val points to an instance of Class SetXFormPacket. See that class for more details on how it is used. 

	  \par Sample Code:
      The following code is from the BoolControl implementation of this method
      (see MAXSDK/samples/controllers/boolcntrl/boolcntrl.cpp).	  
      \snippet samples/controllers/boolcntrl/boolcntrl.cpp Control.GetValue Example
      */
	  ...
	  The following code is from the BoolControl implementation of this method
      (see MAXSDK/samples/controllers/boolcntrl/boolcntrl.cpp).	  
      \snippet samples/controllers/boolcntrl/boolcntrl.cpp Control.SetValue Example
	  */
 ...
 *  The following is an overview of the control flow of the system calling these IK methods
 *  on the controller during IK calculations:
 *  First the system calls EnumIKParams() once to get the number of IK parameters (degrees of freedom)
 *  from the plug-in. This lets the system know how many parameters the controller has. This happens only
 *  once when the entire chain is set up. For example, during interactive IK, when the user presses the mouse
 *  button down, the chain is set up. In applied IK, when the user presses the Apply IK button, the chain is set up.
 *  \n Next, the IK task is defined. For example, if the user is doing interactive IK, they press the mouse
 *  button down and the IK chain is set up. Next, they move the mouse a few pixels. This results in a delta for
 *  the end effector. This defines the task for the end effector. The end effector wants to move some delta or
 *  rotate some amount. This has defined a task to be solved.
 *  \n In order to solve the task the system must iterate towards a solution. To do this it first needs
 *  to compute the derivatives of all the parameters. The system calls CompDerivs() on the controller.
 *  The controller computes the derivative and tells the system by calling methods on the IKDeriv argument
 *  passed to the method. It provides its derivative for each of the parameters.
 *  \n Next, the system uses this derivative information to formulate a change in the parameter that will get
 *  closer to the solution. The system then computes an amount to change the parameter and then
 *  calls IncIKParams(). The delta passed to this method is the amount of change it wants in the parameter.
 *  The controller then applies this change (perhaps not all of it due to constraints it might have) and
 *  returns the amount that was applied.
 *  \n This completes a single iteration. All the parameters have been adjusted a bit and hopefully
 *  we have moved closer to a solution. At this point the system calls CompDerivs() again because just moving
 *  the parameters a small amount may have actually changed the derivative. The cycle begins again. When the
 *  IK solver reaches its solution, the IK task is finished.
 */
///@{
	  // For IK
      ...
      \snippet samples/controllers/layerctrl.cpp Control.EnumIKParams Example
	  */
	  ...
	  linkage that could be non-linear. Therefore the derivative may have only
	  ...
	  /*! \remarks	  User can specify a node as a terminator. This method gives the
...
//! \name Copying and Pasting IK Parameters in the Hierarchy Branch
//! @{	  
...
//! @{	  
	  ...
	  \par Default Implementation:
	  <b>{return REF_SUCCEED;}</b> */
	  virtual RefResult PinNodeChanged(RefMessage message,const Interval& changeInt, PartID &partID) {return REF_SUCCEED;}
//! @}

//! \name Ease/Mulitplier Curves
//! @{
	  // Ease curves
...
//! @}
	  ...
	  so master controllers can easily delete slave nodes if they want to.
	  ...
	  // that ease and multiplier curves are cloned.
	  /*! \remarks	  This method is implemented by the System. It should be called by any leaf
	  controller's <b>Clone()</b> method so that ease and multiplier curves are
	  cloned.
...
/** \name Display a Gizmo (apparatus) in a Scene 
...
///@{
	  ...
	  Default return value is -1, indicates this controller doesn't have display function.
	  \par Default Implementation:
	  <b>{ return 0; }</b> */
	  virtual int Display(TimeValue t, INode* inode, ViewExp *vpt, int flags) { return -1; };
...
/** \name Reference Coordinate System Definition
 * When the user is in a sub-object selection level, the system needs to get the reference coordinate
 * system definition from the current controller being edited so that it can display the axis.
...
//! \name Modify Sub-object Apparatuses
//! @{
	  ...
	  tangents that <b>MapKeys()</b> won't affect and return TRUE if they don't
...
/*! \sa  Class Control, Class Interval, Class Matrix3, Class Point3, \ref Additional_Controller_Related_Functions, Class Quat, Class ScaleValue
\par Description:
...
class StdControl : public Control {
   public:     
	  ...
	  <b>CONTROL.H</b> to do them. See \ref Template_Extrapolation_Functions "Template Extrapolation Functions".
	  ...
	  \par Sample Code:
	  The following sample code shows the implementation in the
      Simple Wave Controller (MAXSDK/samples/controllers/wavectrl/simpwave.cpp).
      \snippet samples/controllers/wavectrl/simpwave.cpp StdControl.Extrapolate Example
	  */
...
class SetXFormPacket: public MaxHeapOperators {
   public:
	  // Warning - instances of this class are saved as a binary blob to scene file by RmTransform::Save
	  // Adding/removing members will break file i/o
...
/*! \remarks Returns a TimeValue that is the specified time mod of the interval length. The returned time is somewhere within the interval passed. This cycles the time so that it appears within the interval.
 */
...
/*! \remarks Returns the number of times the TimeValue cycles through the interval. */
...
//! \name Template Extrapolation Functions
//! \anchor Template_Extrapolation_Functions
//! @{
/*! \remarks Performs a linear extrapolation and returns the result. */
template <class T> T LinearExtrapolate(TimeValue t0, TimeValue t1, const T &val0, const T &val1, const T &endVal)
{
	return (T)(endVal + (val1-val0) * float(t1-t0));
}
/*! \remarks Performs a repeat extrapolation and returns the result. This will cycle the time t into the interval range. */
...
/*! \remarks Performs a linear extrapolation using a slope of one and a point, and returns the result. */
template <class T> T IdentityExtrapolate(TimeValue endPoint, TimeValue t, const T &endVal )
{
...
/*! \remarks Takes an interval and checks the flags and modifies the interval based on the state of the flags. */
...
/*! \remarks Returns a quaternion scaled by the specified factor. This scales the 'angle' of the quaternion. */
...
//! \name Additional Controller Related Functions 
//! \anchor Additional_Controller_Related_Functions 
//! @{
extern CoreExport TempStore tmpStore;   // this should be in the scene data struct.
//! @}

//! \name High-level Animation Control
//! @{
...
//! \name Animation Key Mode Methods
//! @{
