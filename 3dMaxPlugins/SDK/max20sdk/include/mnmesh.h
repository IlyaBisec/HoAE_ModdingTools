...
public:
	...
	/** Allocates enough memory in the arrays for the face to have
	degree d, and then sets the degree. If the arrays are
...
public:
	...
	\see MakePlanar, \see MakeFacePlanar, \see SeparateFace. */
	int track;	
...
class MNMesh : public FlagUser, public BaseInterfaceServer {
	...
	friend class IMNMeshLegacySnap;
...
#pragma region Per-Vertex Data
	...
	\ref per_edge_data "Edge data channel index values". */
	...
	*/
	...
	DllExport int TargetEdgesBySelection (int iSelLevel);	// o(n)
	...
	*/
	...
	/** Returns the surface normal at the edge edgeIndex. This is
	...
	length of 1.\n\n
	This normal is not related to the normals used in rendering. */
	DllExport Point3 GetEdgeNormal (int edgeIndex);	// o(deg)
	...
	DllExport float EdgeAngle (int edgeIndex);
	...
	*/
	...
	\return  The index of the new vertex. */
	DllExport int SplitEdge (int ff, int edgeIndex, float prop, bool right, int *nf=NULL,
	...
	*/
	...
	\param ff - The face to divide.
	\param a - The first index (in the face's vtx list) of the vertices used to divide the
	...
	valid.
	\param b - The second index (in the face's vtx list) of the vertices used to divide the
	face. Note that a and b should be at least two units apart in each
	direction: a can't be (b+1) mod deg, and b can't be (a+1) mod deg. Also,
	the straight line connecting MNVerts V(vtx[a]) and V(vtx[b]) should not
	cross any of the edges of the face, otherwise the results will not be
	valid.
	\param nf - A variable to hold the new face created by this division. (Half the face
	remains as face ff, the other half is the new face nf.)
	\param ne - A variable to hold the new edge created by this division.
	\param neVis - Indicates whether the new edge should be visible. (The other version of
	SeparateFace always leaves the edge invisible.)
	\param neSel - Indicates whether the new edge should be selected. (The other version of
	SeparateFace always leaves the edge unselected.)
	\param track - When true, the two faces resulting from this operation will have the
	same 'track' value as the original so that they can later be associated, if desired.
	*/
	DllExport void SeparateFace (int ff, int a, int b, int & nf, int & ne, bool neVis=FALSE, bool neSel=FALSE, bool track=false);
	...
	\param planarThresh - The planar angle threshold.
	\param track - Indicates whether to track faces that are split by this method.
	When true, the faces of all pieces of any nonplanar faces split by this method
	will have the same 'track' value as the original so that they can later be
	associated, if desired.
	*/
	...
	\param ff - The face of the index to make planar
	\param planarThresh - The planar angle threshold.
	\param track - Indicates whether to track faces that are split by this method.
	When true, the faces of all pieces of any nonplanar faces split by this method
	will have the same 'track' value as the original so that they can later be
...
\endcode  */
	...
	// Use MNMeshUtilities14::GetExtrudeDirection if you want an option to 
	// use the distance perpendicular to the surface versus along the normal
...
/** Only call this method when you need to go back to a previous version of the code to load older files to maintain procedurality */
	...
	void		SetDispFlag(DWORD vflags) { dispFlags |= vflags; }
	/** This method returns the displFlags \& f. See \ref MNDISP_Flags. */
	DWORD		GetDispFlag(DWORD vflags) { return dispFlags & vflags; }
	/** This method allows you to clear the specified display flags. See \ref MNDISP_Flags. */
	void		ClearDispFlag(DWORD vflags) { dispFlags &= ~vflags; }
		//@}
		#pragma endregion
...
public:
	...
	*/
	...
	bool		checkFaceDiagonalSel(int face_i, Mesh_SelParams& params);
	bool		checkVertSel(int vert_i, Mesh_SelParams& params);
	bool		checkFaceBackFacing(int face_i, Mesh_SelParams& params);
	...
	*/
	...
	/** Similiar to above overrided function with the difference of having the barycentric coordinates
	as the function returned result. In case of no hit it will return nullptr. This function guarantees
	the returned array has enough space that matches the degree of the face. It is suggested to use this
	function because it does not require intermediate memory allocation. This function is implemented to
	be thread safe.
	*/
	...
	*/
	...
	It must point to an array of float that have large size than face degree.
	*/
...
\endcode */
class IMNMeshUtilities14 : public BaseInterface {
public:
	Interface_ID	GetID() { return IMNMESHUTILITIES14_INTERFACE_ID; }
