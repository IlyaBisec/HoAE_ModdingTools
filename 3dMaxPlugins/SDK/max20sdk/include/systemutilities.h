...
namespace Util
{
	 ...
	 *
	 * The hMyModule param is used internally for tracking the loading of dlls for debugging purposes, and should not
	 * be specified.
	 * 
	 */
...
/*! This function is used to convert a path relative to the 3ds max install to an absolute path. 
	...
	\param [out] specPath - The resolved absolute path, or null string on failure.

	/returns true if \p specPath was successfully resolved, false otherwise.
	*/
UtilExport bool GetSpecDir(int index, const MCHAR *dirName, MSTR& specPath);	// get directory path
...
/*! This function is a helper for creating standard windows CSIDL directories
	...
	\param [out] csidlPath - The absolute path to the created CSIDL directory, or null string on failure.

	/returns true if \p csidlPath was successfully created, false otherwise.
	*/
UtilExport bool TryCSIDLDir(int csidl, const MCHAR *dirName, MSTR& csidlPath);	// create directory path
...
namespace Util
{
	/* \brief scans a parent folder and all sub-folders for a file
	   ...
	   param[out] targetpath the absolute path to the file you are searching for

	   /returns true if filename is successfully found, false otherwise
	*/
...
namespace MaxSDK {namespace Graphics {
	...
	*/
    ...
    \return true if at least one GPU is present, false otherwise
    */
