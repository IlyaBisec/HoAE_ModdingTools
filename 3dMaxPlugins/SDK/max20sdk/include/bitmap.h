//**************************************************************************/
// Copyright (c) 1994-2012 Autodesk, Inc.
// All rights reserved.
//
...
//**************************************************************************/
/** \file Bitmap.h
	\brief Contains classes and utilities for reading and writing images */
...
//-----------------------------------------------------------------------------
/// \brief The Primary Bitmap Manager Object
...
class bmmHistoryList: public MaxHeapOperators {
	...
	public:
		...
		BMMExport const MCHAR* DefaultPath	( ) { return initDir; }
...
/*! This format is similar to \ref BMM_LOGLUV_24, except the 8 bit alpha value is kept with the 24 bit 
...
#define BMMRES_NUMBEREDFILENAMEERROR  11	//!< This can be passed as an error code to BitmapIO::ProcessImageIOError().
...
Thus this flag is passed to indicate that the user has cancelled and this processing should not occur. */
...
The Pyramidal and Summed Area options provide two methods of pixel averaging that anti-alias the 
...
as a function of distance, irregular anti-aliasing may occur on detailed texture maps that are 
...
typedef BOOL (WINAPI* PBITMAP_FX_CALLBACK)( LPVOID lpparam, int done, int total, const MCHAR* msg );
//-- GRAINEND
...
class BMM_IOHandler : public MaxHeapOperators 
{
   ...
   public:
	  ...
	  BMMExport const MCHAR*           ShortDescription ( const MCHAR*  d = NULL );
	  /*! \remarks Returns the long description string for the IO module, and
	  optionally sets it to <b>d</b> if it is not NULL.
	  \par Parameters:
	  <b>const MCHAR *d = NULL</b>\n\n
	  The long description string to set. */
	  BMMExport const MCHAR*           LongDescription  ( const MCHAR*  d = NULL );
	  ...
	  BMMExport const MCHAR*           Extension        ( int index, const MCHAR*  e = NULL );
...
/*! \sa  Class Bitmap, ~{ Working with Bitmaps }~, \ref bitmapErrorCodes.\n\n
\brief Information about an image file or in-memory bitmap, including settings for loading/saving, gamma, etc.
\par Description:
...
noted otherwise.
\par
In general, BitmapInfo should be used and saved for user-defined files rather
than filenames, because the BitmapInfo encodes information beyond the filename,
including file-format specific settings (e.g. compression on .JPG files) and so on.
\par
The file dialog functions (such as BitmapManager::SelectFileInput() or BitmapManager::SelectFileOutput())
can be seen as "BitmapInfo editors", since they allow the user to edit its content.
*/
...
//@{
		 ...
		 * - when examining the BitmapInfo of a loaded bitmap, this indicates that the file was loaded with the custom gamma (i.e. a user override).
		 * - when using the BitmapManager::SelectFileInput() (or output) file dialogs, it indicates the "Custom" gamma option.
		 * - when set before loading Bitmap files (calling BitmapManager::Load()), it indicates that the gamma should be read using the custom gamma.
		 * - when set before opening Bitmap files for write (calling Bitmap::OpenOutput()), it indicates that the file should be written with the custom gamma.
		 */
		...
		/*! 
		 *  This flag can be used in several cases:
		 * - when examining the BitmapInfo of a loaded bitmap, this indicates that the gamma came from the file (as opposed from a user override or the defaults).
		 *   See also BMM_CUSTOM_INFERREDGAMMA.
		 * - when using the BitmapManager::SelectFileInput() (or output) file dialogs, it indicates the "Automatic" gamma correction mode
		 * - when set before loading Bitmap files (calling BitmapManager::Load()), it indicates that the gamma should be read from file (if possible) and inferred from the file-type if not.
		...
		//! Additional flag to BMM_CUSTOM_FILEGAMMA. Did gamma actuallly come from data stored in the file, or was it inferred from the file type?
		/*! 
		    If BMM_CUSTOM_FILEGAMMA is set prior to loading a file (calling BitmapManager::Load()), the BitmapManager will attempt to load a file
		    using gamma information stored <i>in the file header</i>. After load, BMM_CUSTOM_FILEGAMMA will be set and
		    the Bitmap's BitmapInfo::Gamma() value will be set to the appropriate gamma.
		    - If only BMM_CUSTOM_FILEGAMMA is set, it means that the gamma truly came from a stored value in the
		      file header. 
		    - If, in addition, the BMM_CUSTOM_INFERREDGAMMA is set, this indicates the case where
		      no actual gamma was stored in the file header, but gamma was computed based on the automatic
		      gamma correction feature, i.e. "inferred" from the file type, bit depth, etc.
		...
		/*!
		* This can be used when the BitmapManager::SelectFileInput() or BitmapManager::SelectFileOutput() are used to request <i>only a filename</i>. 
		* This avoids the user being confused by an UI that does nothing and has no effect.
		*/
...
//! @{
		...
		//-- "Device" is the device responsible for producing this image.
		...
		\remarks This copies the supplied string into the member variable device.
		The string corresponds with the long description of a BitmapIO plugin: 
		BitmapIO::LongDesc(). 
		However it is not necessary to call this as the system will do it automatically
		when Bitmap::OpenOutput is called. Or as alternative, it can properly be set in a 
		localized environment by calling BitmapInfo::FixDeviceName(). 
		\param d - The name to set.
...
//! @}
		...
		/**
		* Returns the plugin data stored on the BitmapInfo.  The plugin data is set by BitmapIO class 
		* responsible for producing this image. The BitmapIO class stores its image specific data in the plugin data.
		* For example, the avi BitmapIO class stores the compressor name, quality, and keyframe in the plugin data.
		* The BitmapInfo owns the memory associated with the plugin data and will free the memory when deleted.
		* \return  Pointer to the plugin data.
		*/
		BMMExport void*			GetPiData                   ( ) { return pidata;          }
		/**
		* Sets the plugin data stored on the BitmapInfo.  This method is not normally called, rather a
		* BitmapIO instance will call AllocPiData(), which will allocate the plugin data memory and set the plugin data 
		* size. The BitmapIO will then acquire the pointer to the plugin data memory via GetPiData()
		* \param[in] ptr Pointer to the plugin data.
		*/
		BMMExport void          SetPiData                   ( void *ptr ) { pidata = ptr; }
		/**
		* Returns the size of the plugin data stored on the BitmapInfo.
		* \return  Size of the plugin data.
		*/
		BMMExport DWORD         GetPiDataSize               ( ) { return pisize;          }
		/**
		* Sets the reported size of the plugin data stored on the BitmapInfo.  This method is not normally called, rather a
		* BitmapIO instance will call AllocPiData(), which will allocate the plugin data memory and set the plugin data 
		* size. The BitmapIO will then acquire the pointer to the plugin data memory via GetPiData()
		* \param[in] s Reported size of the plugin data.
		*/
		BMMExport void          SetPiDataSize               ( DWORD s ) { pisize = s;     }
		/**
		* Resets the plugin data stored on the BitmapInfo. Frees existing plugin data memory and sets plugin data size to zero.
		*/
		BMMExport void          ResetPiData                 ( );
		/**
		* Allocates the plugin data memory and set the plugin data size. Acquire the pointer to the plugin data memory via GetPiData()
		* \param[in] size Requested size of the plugin data.
		*/
...
//! @}
	...
	private:
		/**
		* This is used internally - the system calls this method when the BitmapInfo is loaded. It creates an instance of the
		* BitmapIO and calls ValidatePiData() on that passing in the BitmapInfo instance and the ILoad*. The BitmapIO can
		* perform versioning of the plugin data, storing it back on the BitmapInfo.
		* \param[in] iload The ILoad*.
		* \see BitmapIO
		*/
		void          UpdateFmtSpecBlockOnLoad    ( ILoad *iload );
};
...
/*! \sa  Class Bitmap, Class BitmapStorage, Class BitmapInfo, Class BitmapIOMetaData, ~{ Working with Bitmaps }~.\n\n
\brief Base class for image IO plugins.
\par Description:
...
requested.
\par Automatic gamma correction / BitmapIOMetaData
If a particular BitmapIO is intended to read/write floating point data, or
in general does not want to have the data encoded with a gamma curve, it can
choose to expose the BitmapIOMetaData interface.
This is done by exposing an interface with the ID of BITMAPIOMETADATA_INTERFACE_ID
(retrievable via <b>BitmapIO::GetInterface(BITMAPIOMETADATA_INTERFACE_ID)</b>).
This returns a pointer to the BitmapIO's BitmapIOMetaData interface. 
This exposes a function BitmapIOMetaData::UseLinearStorage() that will return 
true if that BitmapIO would like to (by default) load and save data linearly 
...
class BitmapIO : public BaseInterfaceServer {
   ...
   protected:
	  float                   gamma;        //!< The gamma setting.
	  Bitmap                  *map;         //!< The Bitmap using this OUTPUT handler
	  BitmapStorage           *storage;     //!< The BitmapStorage used by this INPUT handler

	  int                      openMode;    //!< The mode that the IO module is open for - see \ref bitmapOpenModeTypes.
	  
	  //-- Linked list pointers for multiple output of a single bitmap

	  BitmapIO                *prevIO;      //!< A linked list pointer to the previous IO module for multiple outputs of a single bitmap.
	  BitmapIO                *nextIO;      //!< A linked list pointer to the next IO module for multiple outputs of a single bitmap.
...
//! @{
	  ...
	  Setting this parameter to FALSE will cause pixels with non-pre-multiplied
...
//! @}
	  ...
	  BitmapInfo               bi; //!< Describes the properties of the bitmap being handled by the instance of this class.

//! \name Open Mode Setting
//! @{
	  /*! \remarks Implemented by the System.\n\n
...
//! @{
	  ...
	  */
	  BMMExport virtual  int            ExtCount                    ( ) = 0;                                // Number of extensions supported
...
//! \name Author/Desc/Copyright/Version
//! @{
...
//! \name ShowAbout / ShowImage / ShowControl
//! @{
	  ...
	  <b>{ return FALSE; }</b> */
	  BMMExport virtual  BOOL           ShowImage                   ( HWND hWnd, BitmapInfo *pbi ) { return FALSE; }
...
/** \name BitmapIO Methods
 * The following methods (EvaluateConfigure(), LoadConfigure(), and SaveConfigure()) deal
 * with BitmapIO loading and saving. See the sample code below to know how the
 * EPS BitmapIO plug-in uses these methods.
 * \code
...
///@{
	  ...
	  //  BitmapIO configuration/option parameters to the host ( ptr is a pre-allocated buffer).
	  //
	  //  LoadConfigure() will be called so the plug-in can load its 
	  //  BitmapIO configuration/option parameters back.
	  //  
	  ...
	  configuration data.
	  \param ptr - Pointer initialized to point to the previously saved configuration data.
	  \param piDataSize - The size of the previously saved configuration data.
	  \return  Returns TRUE if the data was loaded properly; otherwise FALSE.
	  */
	  BMMExport virtual  BOOL           LoadConfigure               ( void *ptr, DWORD piDataSize ) = 0;
	  /*! \remarks This method is called by 3ds Max to allow the plug-in to save any
	  configuration data.
	  \param ptr - Pointer initialized to a pre-allocated buffer where the plug-in may save
	  data.
 ...
 * The options control how files are saved to disk (JPEG compression and smoothing, for example).
 * Formerly, most BitmapIO types implemented these methods, yet they were not derived from any superclass.
 * The method definitions are now moved to the BitmapIO superclass, so they can easily be called on any subclass.
...
///@{
	  ...
	  options. */
	  BMMExport virtual  void           GetCfgFilename              ( MCHAR *filename ) { filename[0]=0; }
...
///@}
	  ...
	  /**
	  * This is used to make sure plugin data on the BitmapInfo belongs to this plugin. It is also called on file load to allow the plugin
	  * to version the plugin data. In this case, the iload parameter will be non-NULL. If a plugin implements this method, it must first call the method
	  * on BitmapIO and immediately return false if that implementation returns false. 
	  * \code
	  ...
	  * }
	  * \endcode
	  * \param[in] pbi The BitmapInfo holding the plugin data
	  * \param[in] iload The ILoad* if called when loading the BitmapInfo from a scene file, NULL otherwise.
	  */
	  BMMExport virtual  BOOL           ValidatePiData              ( BitmapInfo *pbi, ILoad *iload = NULL  );

//! \name Silent Mode Setting
//! @{
	  //-- System Interface
...
//! @{
	  ...
	  name (using <b>pbi-\>Name()</b> or <b>pbi-\>Device()</b>).\n\n
      ...
      \snippet samples/images/jpeg/jpeg.cpp BitmapIO.ProcessImageIOError Example
	  */
	  BMMExport BMMRES         ProcessImageIOError         ( BitmapInfo *pbi, int errorcode);
//! @}

//! \name G-Buffer Channels Required
//! @{
	  //---------------------------------------------------------------------
...
//! @{
	  ...
	  BMMExport virtual  BMMRES         GetImageInfoDlg             ( HWND hWnd, BitmapInfo *pbi, const MCHAR *filename = NULL ) {return BMMRES_NODRIVER;}
	  ...
	  \par Sample Code:
		...
		return BMM_SUCCESS;
		\endcode */
	  BMMExport virtual  BMMRES         GetImageInfo                ( BitmapInfo *pbi ) = 0;
	  ...
	  BMMExport virtual  BMMRES         GetImageName                ( BitmapInfo *pbi, MCHAR *filename) {filename[0]=0; return (BMMRES_SUCCESS);}
	  ...
	  frame" parameters and returns it so 3ds Max can compare it with another
...
//! @{
	  ...
	  If the image header contains known gamma information, it can call
	  SetGamma() with the relevant value.
	  ...
	  \par Sample Code:
	  See the <b>Load()</b> method of <b>/MAXSDK/samples/images/jpeg/jpeg.cpp</b>.
	  */
	  BMMExport virtual  BitmapStorage *Load                        ( BitmapInfo *pbi, Bitmap *pmap, BMMRES *status ) = 0;   
//! @}

//! \name Output / Write / Close
//! @{
		/*! \brief Opens the image for writing to file.
		\remarks This method opens the image for output and prepares to write to it. This is the
		time that the plug-in receives the information about what to write, the flags,
		etc. 
		The only thing to call after this the Write() method which takes just a frame number.
		\param pbi - The image information.
		\param pmap - Points to the bitmap to save.
		\return  Returns nonzero if everything was OK; otherwise 0.
		\par Sample Code:
		See the <b>OpenOutput()</b> method of <b>/MAXSDK/samples/images/jpeg/jpeg.cpp</b>. */
	  BMMExport virtual  BMMRES         OpenOutput                  ( BitmapInfo *pbi, Bitmap *pmap );
		...
		\return  Returns nonzero if everything was OK; otherwise 0.
		\par Sample Code:
		See the <b>Write()</b> method of <b>/MAXSDK/samples/images/jpeg/jpeg.cpp</b>. */
      ...
      \snippet samples/images/gif/gif.cpp BitmapIO.Close Example	  
	  */
...
class BitmapStorage : public BaseInterfaceServer {
   ...
   public:
	  ...
	  */
	  ...
	  zero. See the Advanced Topics section
	  ~{ Working With Bitmaps }~ for details on High Dynamic Range bitmaps. */
	  ...
	  \par Default Implementation:
	  <b>{ *bmmType = BMM_NO_TYPE; return (NULL); }</b> */
	  BMMExport virtual void           *GetStoragePtr               ( int *bmmType ) { *bmmType = BMM_NO_TYPE; return (NULL); }
	  ...
	  width of the image. */
	  BMMExport virtual void           *GetAlphaPtr                 ( int *bmmType) { *bmmType = BMM_NO_TYPE; return (NULL); }
	  ...
	  argument -- for simple copy operations, <b>*bmInfo</b> can default to NULL.
	  If present, the code checks the option flags and acts accordingly.
	  \return  Nonzero if the copy was performed; otherwise 0. */
	  BMMExport virtual int             CopyImage                   ( Bitmap *from,int operation,BMM_Color_64 fillcolor, BitmapInfo *bmInfo = NULL) = 0;
	  /*! \remarks Copies the specified bitmap to this storage. The image is cropped or
	  resized as specified.
	  \par Parameters:
	  <b>Bitmap *from</b>\n\n
	  ...
	  If present, the code checks the option flags and acts accordingly.
	  \return  Nonzero if the copy was performed; otherwise 0. */
	  BMMExport virtual int             CopyImage                   ( Bitmap *from,int operation,BMM_Color_fl fillcolor, BitmapInfo *bmInfo = NULL) = 0;
      ...
      \snippet samples/images/bmp/bmp.cpp Bitmap.SetPalette Example
	  */
...
Note that <b>bVirtual</b> is actually a short-cut macro for <b>BMMExport
virtual</b>.\n\n
...
class BitmapStorageLDR : public BitmapStorage {
   public:
	  ...
	  operation specified is <b>COPY_IMAGE_CROP</b> and one of the source
	  bitmap dimensions is less than the size of this bitmap.\n\n
	  <b>BitmapInfo *bmInfo = NULL</b>\n\n
	  When using custom options (resize to fit, positioning, etc.) this is how
	  the flags are passed down to the Bitmap Manager. This is an optional
	  argument -- for simple copy operations, <b>*bmInfo</b> can default to NULL.
	  If present, the code checks the option flags and acts accordingly.
	  \return  Nonzero if the copy was performed; otherwise 0. */
	  BMMExport virtual int             CopyImage                   ( Bitmap *from,int operation,BMM_Color_64 fillcolor, BitmapInfo *bmInfo = NULL);
	  /*! \remarks Copies the specified bitmap to this storage.
	  \par Parameters:
	  <b>Bitmap *from</b>\n\n
	  The source bitmap.\n\n
	  ...
	  \return  Nonzero if the copy was performed; otherwise 0.\n\n
		*/
	  BMMExport virtual int             CopyImage                   ( Bitmap *from,int operation,BMM_Color_fl fillcolor, BitmapInfo *bmInfo = NULL);
	  /*! \remarks Copies the specified bitmap to this storage.
	  \par Parameters:
	  <b>Bitmap *from</b>\n\n
	  The source bitmap.\n\n
...
/*! \brief Class for image input and output.
\par Description:
The Bitmap class is the bitmap itself. All image access is done through
this class. The Bitmap class has methods to retrieve properties of the
...
\sa  Class BitmapManager, Class BitmapInfo, Class BitmapStorage, Class GBuffer, ~{ Working with Bitmaps }~.\n\n
*/
 ...
 * The following methods return properties of the bitmap. When these methods are called, they are passed
 * to the BitmapStorage instance maintained by the Bitmap. The BitmapStorage implements these methods by returning
 * the properties of the BitmapInfo instance maintained by the storage. Therefore, the values returned from the
 * methods represent what is returned from the BitmapInfo instance associated with the Bitmap.
 */
///@{
	  ...
	  \return  If the BitmapInfo instance maintained by the storage. */
	  inline  BitmapInfo              GetBitmapInfo        ( ) { if (storage) return storage->bi;      return BitmapInfo(); };

///@}

//! \name Output / Writing / Closing Methods
//! @{
	  /*! \remarks	  Implemented by the System.\n\n
	  Set whether colors are scaled (on) or clamped (off) when converting from
...
/** \name Standard Pixel Access Methods
...
//! \name Filtering / Dithering Methods
//! @{
...
//! \name Output / Writing / Closing Methods
//! @{
	  /*! \brief Opens the image for output
	  \remarks This method will open the image for output. This allows the
	  image to be written to. Note that you can pass a BitmapInfo to
	  this method on which you simply set the file name 'by hand' 
	  (<b>bi-\>SetName(_T("D:\renders\output.jpg"))</b>). 
	  This will work and the correct driver (i.e. BitmapIO) will be
	  selected based on the filename extension. This will work even in a localized environment,
	  therefore there is no need to manually call BitmapInfo::SetDevice on the BitmapInfo that is passed
	  to this function. However you won't be able to set any driver specific settings (such as 
	  compression settings for JPEGs). The alternative way is to use the BitmapManger methods.
	  \param bi - Contains the name of the image or device to open for output.
	  \return  
	  <b>BMMRES_SUCCESS</b> - Indicates success.\n\n
	  <b>BMMRES_ERRORTAKENCARE</b> - Indicates that 3ds Max could not find a device to handle the image.\n\n
	  <b>BMMRES_INTERNALERROR</b> - Indicates the IO module handling the image could not be opened for
	  writing. */
	  BMMExport BMMRES         OpenOutput                  ( BitmapInfo *bi );

		/*! \brief Write the image from BitmapStorage to disk.
		\remarks Write the image from the BitmapStorage to disk. Note that
		you must pass the same BitmapInfo used when the file was first "Opened
		for Output". The main reason is that any device specific settings are kept in
		the BitmapInfo object. Also, the custom options such as gamma value,
		optional channels, etc (if any) are kept in this BitmapInfo instance.
		These are assigned when the bitmap is first opened (for either read or write).
		\param bi - Contains the name of the file or device to write to.
		\param frame - Specifies the frame number to write. If this is a single image, allow
		frame to default to single frame. This argument determines if the file
		...
		to BMM_SINGLEFRAME (the default). Any other value is considered to be a frame number and it
		will be appended to the given filename.
		\return  <b>BMMRES_SUCCESS</b> - Indicates success.\n\n
		<b>BMMRES_ERRORTAKENCARE</b> - Indicates that 3ds Max could not find a device to handle the image.\n\n
		<b>BMMRES_INTERNALERROR</b> - Indicates the IO module handling the image could not be opened for writing. */
...
//! \name Display (Virtual Frame Buffer) Methods
//! @{
	  ...
	  BMMExport int            Display                     ( const MCHAR *title = NULL, 	int position = BMM_CN, 
...
//! \name Storage Change Notification
//! @{	  
...
//! \name Display (Virtual Frame Buffer) Methods
//! @{	  
	  /*! \remarks	  Returns TRUE if the virtual frame buffer (VFB) is autonomous; otherwise
...
/*! \brief Custom browse bitmap file for input dialog.
\sa Class BitmapManager.\n\n
\par Description:
This class can be used to replace the standard 3ds Max Browse Images for Input dialog such as:
Select Bitmap Image File dialog.
*/
class BitmapFileInputDialog: public MaxHeapOperators {
	public:
		/*! \remarks This method is called in BitmapManager::SelectFileInput() (or BitmapManager::SelectFileInputEx())
		which will bring up the custom browse images for input dialog box. 
		\par Parameters:
		See BitmapManager::SelectFileInput().
		\return  TRUE if the user exited the dialog using OK; otherwise FALSE.
		*/
		virtual BOOL BrowseBitmapFilesInput(BitmapInfo* info, HWND hWnd, const MCHAR* title, BOOL view) = 0;
	};

/*! \brief Custom browse bitmap file for output dialog.
\sa Class BitmapManager.\n\n
\par Description:
This class can be used to replace the standard 3ds Max Browse Images for Output dialog such as:
Render Output File dialog, Create Preview Animation Output File dialog.
*/
class BitmapFileOutputDialog: public MaxHeapOperators {
	public:
		/*! \remarks This method is called in BitmapManager::SelectFileOutput()
		which will bring up the custom browse images for output dialog box. 
		\par Parameters:
		See BitmapManager::SelectFileOutput().
		\return  TRUE if the user exited the dialog using OK; otherwise FALSE.
		*/
		virtual BOOL BrowseBitmapFilesOutput(BitmapInfo* info, HWND hWnd, const MCHAR* title,
                                                                          const MCHAR *extension = NULL,
                                                                          const MCHAR *config = NULL,
                                                                          const MCHAR *map = NULL) = 0;
	};
...
/*! \sa  Class BitmapInfo, Class Bitmap, Class BitmapIO, Class BitmapStorage, ~{ Working with Bitmaps }~, \ref bitmapErrorCodes.\n\n
\par Description:
...
//! @{
	  ...
	  /*! Sets the silent mode on/off. The silent mode disables any message box pop-ups that may interrupt the process.
        \return The previous state of the silent mode.*/
...
//! @{
	  ...
	  section ~{ Working with Bitmaps }~. Make sure you delete the Bitmap created when you are done
	  using it.
	  \par Parameters:
	  <b>BitmapInfo *bi</b>\n\n
	  ...
	  section ~{ Working with Bitmaps }~.
	  <b>bool setGamma</b>\n\n
	  If true, the Bitmap is created with its BitmapInfo's custom flag set to 
	  BMM_CUSTOM_GAMMA and its gamma set to the current display gamma
	  ...
	  */
	  BMMExport virtual Bitmap        *Create                      ( PBITMAPINFO pbmi, bool setGamma = false  )=0;
...
//! @{
	  ...
	  <b>const MCHAR *extension = NULL</b>\n\n
	  The optional extension string to display in the Save as type field of the dialog.\n\n
	  <b>const MCHAR *config = NULL</b>\n\n
	  The optional name template configuration, used to load according name templates 
	  and display in Name template dropdown combobox of the dialog. 
	  Currently support only two name template configurations: PreviewNTP (the default) and TextureNTP. 
	  The name templates for these configurations are:\n\n
	  <b>PreviewNTP:</b>\n\n
	  <b>\<scene\></b>\n\n
	  <b>\<scene\>_\<camera/view\></b>\n\n
	  <b>\<scene\>_\<camera/view\>_\<mm\>-\<dd\></b>\n\n
	  <b>\<scene\>_\<camera/view\>_\<mm\>-\<dd\>-\<yyyy\></b>\n\n
	  <b>CustomTemplate_\<scene\>_\<camera/view\>_\<mm\>-\<dd\></b>\n\n
	  <b>TextureNTP:</b>\n\n
	  <b>\<object\>_\<map\></b>\n\n
	  <b>CustomTemplate_\<object\>_\<map\></b>\n\n
...
//! \name Display / VFB Related Methods
//! @{
...
extern           void            ExtractExtension	( const MCHAR *string, MCHAR *ext );
...
/*! \brief Create a filename appended by the specified frame number. This function will pad the filename with zeroes.
	...
	\param[in] frame: The frame number to append. Can be BMM_SINGLEFRAME for convenience, which results in the original filename.
...
This is similar to the sequence detection used in the Image File List (IFL) image format.
	\param[in] baseFilename: The filename to match.
...
//! \brief Splits up a file path into three strings, the directory name, file name, and file extension.
/*! The three out parameters are allowed to be NULL. For non-NULL parameters, it is also assumed 
	the memory pointed to by the three parameters is big enough to hold the corresponding string contents. 
	\param[in]  name       - The full path to the file to be split
	\param[out] directory - A string containing the directory portion of the file path
	...
	public:
		...
		*/
		...
		/**
			...
			When Memory Limit Auto Mode is enabled, the Memory Limit Percentage is overwritten on every Auto Mode update. 
		...
		*/
		...
		/**
			...
			The adjustment happens when the physical limit is calculated, during file load, file reset, render begin and render end.
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
//-- BitmapIOMetaData (for gamma/colorspace info )
//

//! \brief Interface ID of the BitmapIOMetaData interface
#define BITMAPIOMETADATA_INTERFACE_ID Interface_ID(0x56507912, 0x7e3c56db)

/*!
\brief Extends BitmapIO with additional information for automatic gamma correction etc.
\sa  Class BitmapIO
\par Description:
This class exposes extended parameters for the BitmapIO class that relates to things
like gamma correction preferences. It is returned as an extended interface of BitmapIO,
returned by calling <b>GetInterface(BITMAPIOMETADATA_INTERFACE_ID)</b> on it.
\par
If this interface is exposed, it can make a decision whether that BitmapIO implementation
would prefer to save or load data linearly. It is called by the code in 
BitmapManager::Load() when loading and Bitmap::OpenOutput() when saving.
\par
Any BitmapIO that does <i>not</i> expose this interface is simply assumed to use
the default gamma value when reading and writing the files (the sRGB gamma of 2.2), very 
similar to how things worked in older 3ds Max versions.
*/
class BitmapIOMetaData : public BaseInterface {
	public:
		//! \brief Does this BitmapIO prefer linear data or not?
		//! \param save is true if we are saving, false for loading.
		//! \return <i>true</i> if we prefer linear storage, <i>false</i> if we want to use the the default - generally sRGB - gamma.
		/*! 
			\par Usage:
			If this BitmapIO always wants to work with linear data, this function can just blindly return <i>true</i>.
			If, however, it can support <i>both</i> float formats and non-float formats depending on settings 
			(like, for example, the DDS or RPF plugins), it needs to be able to make decisions on what to return. 
			\par
			To do this properly, one needs to understand the calling sequence; this function is called in two cases:
			- From BitmapManager::Load() (with save set to <i>false</i>). Since this is called <i>after</i> loading, 
			  it is legal to make decisions based on the BitmapIO::bi member, for example examining <b>BitmapIO::bi.Type()</b>.
			- From Bitmap::OpenOutput() (with save set to <i>true</i>). Decisions need to be made based on the configuration 
			  of the bitmap plugin, since this is called <i>before</i> saving, the BitmapIO::bi member is in this case <i>not</i> valid.

			An example of this is available in <b>/MAXSDK/SAMPLES/IMAGES/RLA/RLA.CPP</b>.
		*/
