...
// per parameter block descriptor flags
#define P_CLASS_PARAMS		0x0001		//!< this block holds class-level parameters, attached to ClassDesc
#define P_AUTO_CONSTRUCT	0x0002		//!< instructs ClassDesc2 to auto-construct this block & wire it in, requires parameter block reference number
...
#define P_MULTIMAP			0x0020		//!< indicates this block as multiple parameter maps   ### move me
#define P_CALLSETS_ON_LOAD	0x0040		//!< causes CallSets() to be called during load PLCB for this block
#define P_HASCATEGORY		0x0080		//!< indicates, that category field is defined for rollup (after rollup proc)
...
#define P_VERSION			0x0200		//!< indicates a version number is specified (integer value, before the reference number field)
#define	P_AUTO_UI_QT		0x0400		//!< This block supports automatic UI management through Qt. Requires the class descriptor, and the implementation of ClassDesc2::CreateQtWidget(). Incompatible with P_AUTO_UI and P_TEMPLATE_UI. See: \ref parammap_qt.
...
#define	P_LOAD_AS_TEMPORARY	0x4000		//!< This block is loaded as a dynamic param block. A plugin has to set a ClassDesc2 later on that block, during a postLoad callback, e.g.

// per parameter constructor-specifiable flags (flag value 1)
#define P_ANIMATABLE		0x00000001	//!< animatable parameter
...
#define P_SUBANIM			0x00000040	//!< non-animatable reference parameter is still a subanim (makes it visible in TV)
#define P_TV_SHOW_ALL		0x00000080	//!< for Tab<> animatables, show all entries even if no controller assigned
#define P_NO_REF			0x00000100	//!< for reference target params do not maintain Reference automatically
#define P_OWNERS_REF		0x00000200	//!< reference parameter maintained by owner, specify owner's reference number via the p_refno tag
...
#define P_SUBTEX			0x00000800	//!< indicates texmap parameter is kept by owner using MtlBase::xSubTexmap protocol, give subtex # in p_subtexno
#define P_VARIABLE_SIZE		0x00001000	//!< Tab<> parameter is variable size allowing scripted changes
...
/* When used as a ParamBlockDesc2 parameter flag, the parameter will not be saved to disk or maintain references or animation keys.
...
#define P_OBSOLETE		    0x40000000	// obsolete parameter, don't maintain refs, a controller or a tab, and don't save to file

//! \brief Indicates that a second per parameter constructor-specifiable flag value follows the first in the ParamBlockDesc2.
/* If this flag is set on the first per parameter constructor-specifiable flag value, a second DWORD size per parameter constructor-specifiable 
...
This flag is specified in the second per parameter constructor-specifiable flag value*/
...
This flag has no effect for parameters types other than TYPE_INODE and TYPE_INODE_TAB. 
This flag needs to be specified as the second flag value when declaring ParameterBlock2 parameters (see P_READ_SECOND_FLAG_VALUE). */
...
/** Indicates that the parameter block's validity interval is intersected with that of the Node parameter's 
object state (including the world space modifiers). This flag has no effect for parameters types 
other than TYPE_INODE and TYPE_INODE_TAB. This flag needs to be specified as the second flag value when 
declaring ParameterBlock2 parameters (see P_READ_SECOND_FLAG_VALUE). */
...
/** Indicates that changes to this parameter will not be recorded by the macro recorder. Useful when using P_USE_ACCESSOR_ONLY entries to control other parameters indirectly.
This flag needs to be specified as the second flag value when declaring ParameterBlock2 parameters (see P_READ_SECOND_FLAG_VALUE). */
...
For example, the plugin will not need to override Animatable::EnumAuxFiles() in order for the parameter's value to show up
in the Asset Tracker System, to be archived, etc. This flag needs to be specified as the second flag value when declaring 
ParameterBlock2 parameters (see P_READ_SECOND_FLAG_VALUE). */
...
By setting this flag, the parameter cannot be retargetted via the Asset Tracking System, and cannot be edited via Maxscript.
This flag needs to be specified as the second flag value when declaring ParameterBlock2 parameters (see P_READ_SECOND_FLAG_VALUE). */
...
// per parameter internal flags
#define P_IS_REF			0x0000000000010000 //!< is a reference target parameter
#define P_HAS_DEFAULT		0x0000000000020000 //!< parameter has a default value
...
#define P_UI_ENABLED		0x0000000000800000 //!< indicates whether UI controls are initially enabled or disabled
#define P_HAS_PROMPT	    0x0000000001000000 //!< has status line prompt string res ID for various picker buttons
#define P_HAS_CAPTION	    0x0000000002000000 //!< has caption string res ID for open/save file dialogs
#define P_HAS_FILETYPES	    0x0000000004000000 //!< has file types string res ID for open/save file dialogs (in MAXScript type: form)
#define P_HAS_REFNO		    0x0000000008000000 //!< has reference number supplied
#define P_HAS_SUBTEXNO	    0x0000000010000000 //!< has sub-texture number supplied
...
#define P_DEFAULTS_AND_RANGES_IN_METERS 0x0008000000000000 //!< p_defaults_and_ranges_in_meters tag specified
...
#define root_value_type(t)	((ParamType2)((t) & ~(TYPE_BY_VAL | TYPE_BY_REF | TYPE_BY_PTR)))	//!< get base type ignoring all flags except Tab
#define is_tab(t)		((t) & TYPE_TAB)					//!< is this parameter a table?
#define is_by_val(t)	((t) & TYPE_BY_VAL)					//!< is this parameter passed by value?  (only for function publishing)
#define is_by_ref(t)	((t) & TYPE_BY_REF)					//!< is this parameter passed by reference?  (only for function publishing)
#define is_by_ptr(t)	((t) & TYPE_BY_PTR)					//!< is this parameter passed by pointer?  (only for function publishing)
#define is_ref(d)		(((d).flags & (P_IS_REF | P_NO_REF | P_OWNERS_REF)) == P_IS_REF) //!< is this parameter a true local reference maker?
#define has_ui(d)		((d).ctrl_count > 0)				//!< this parameter has UI info defined
...
struct PB2Value: public MaxHeapOperators {
	union
	{
		...
		Control*				control;  // replaces i, f, p, p4, p2, or t values if animated
	};
	...
	\param type The type of value held by the PB2Value. */
	...
	\param type The type of value held by a PB2Value. */
	PB2Export static bool RequiresFree(ParamType2 type);

	// Helper method for replacing a string in a PB2Value instance
...
\remarks This structure provides information about a parameter alias. Aliases 
...
/*! \sa  Class ReferenceTarget, Class ParamBlockDesc2, ::ParamType, ::ParamType2, 
...
{
public:
//! \name Version/Parameter Number/Local Name/ParamDef/BlockID/Owner Access
//! @{	
	...
	*/
	...
	virtual const MCHAR*		GetLocalName()=0;
	...
	*/
	...
	\param parameterIndex Index of the parameter to be fetched. 
	\return A pointer to the parameter definition for the given index, or null if the index is invalid. */
	const ParamDef* GetParamDefByIndex(const unsigned int parameterIndex) const;
//! @}

//! \name Descriptor Access
//! @{
	// acquire & release the descriptor for this paramblock, get individual paramdefs
	/*! \remarks Acquires the descriptor for this parameter block. Call
...
//! \name Super Class ID Access
//! @{
	// Gets the super class of a parameters controller
...
//! \name Parameter Type and Local Name
//! @{
...
//! \name SetValue() Parameter Accessors 
//! @{	
	...
	*/
	...
	/*! \remarks Sets the Point2 value of the specified parameter at the
	specified time.
	\param id The permanent ID of the parameter.
	\param t The time at which to set the value.
	\param v The value to set.
	...
	\return  TRUE on success; otherwise FALSE.
	\note The ParamType must be one of the following, otherwise no set occurs:
	TYPE_POINT2, TYPE_INT2, TYPE_BOOL2
	*/
	...
	*/
	...
	TYPE_POINT3, TYPE_RGBA, TYPE_HSV
	*/
	virtual BOOL SetValue(ParamID id, TimeValue t, const Color& v, int tabIndex=0)=0;  // uses Point3 controller
	...
	TYPE_POINT4, TYPE_FRGBA
	*/
	virtual BOOL SetValue(ParamID id, TimeValue t, const AColor& v, int tabIndex=0)=0;  // uses Point4 controller
...
//! \name GetValue() Parameter Accessors
//! @{	
	...
	*/
	...
	/*! \remarks Retrieves the Point2 value of the specified parameter at
	the specified time.
	\param id The permanent ID of the parameter.
	\param t The time at which to get the value.
	\param v The value to retrieve is returned here.
	...
	table of the value to get.
	\return  TRUE on success; otherwise FALSE.
	\note The ParamType must be one of the following, otherwise no get occurs and v will contain Point2::Origin:
	TYPE_POINT2, TYPE_INT2, TYPE_BOOL2
	*/
	...
	*/
	...
	\param t The time at which to get the value.
	\param v The value to retrieve is returned here.
	\param [out] ivalid Validity interval of the node parameter. It's updated (intersected) by the validity 
	of certain aspects of the node, such as geometry and transform, as specified in the parameter descriptor. 
	See P_USE_NODE_OS_VALIDITY and P_USE_NODE_TM_VALIDITY for more details. This validity interval does 
	not pertain to the validity of the node pointer.
	\param tabIndex If the parameter is a Tab\<\> this is the zero based index into the table of
	the value to get.
	\return  TRUE on success; otherwise FALSE.
	\note The ParamType must be one of the following, otherwise no get occurs and v will contain NULL:
...
//! @{
	...
	TYPE_POINT3, TYPE_RGBA, TYPE_HSV
	\note Starting in 3ds Max 2016, please use \ref IParamBlock2::GetColor(ParamID,TimeValue,Interval&,int) instead; it supports a validity interval
	and is more robust in case of invalid parameter IDs.
	*/
	...
	*/
	...
	\note The ParamType must be one of the following, otherwise no get occurs and a value of black returned:
	TYPE_POINT4, TYPE_FRGBA
	\note Starting in 3ds Max 2016, please use \ref IParamBlock2::GetAColor(ParamID,TimeValue,Interval&,int) instead; it supports a validity interval
	and is more robust in case of invalid parameter IDs.
	*/
	...
	*/
	...
	/*! \remarks Returns the Point2 value of the specified parameter at the
	specified time.
	\param id The permanent ID of the parameter.
	\param t The time at which to get the value.
	\param tabIndex If the parameter is a Tab\<\> this is the zero based index into the
	table of the value to get.
	\return  The Point2 value retrieved.
	\note The ParamType must be one of the following, otherwise no get occurs and a value of Point2::Origin returned:
	TYPE_POINT2, TYPE_INT2, TYPE_BOOL2
	\note Please use \ref IParamBlock2::GetPoint2(ParamID,TimeValue,Interval&,int) instead; it supports a validity interval
	and is more robust in case of invalid parameter IDs.
	*/
	virtual Point2 GetPoint2(ParamID id, TimeValue t=0, int tabIndex=0)=0;

	/*! \remarks Retrieves the Point2 value of the specified parameter at the
	specified time.
	\param id The permanent ID of the parameter.
	\param t The time at which to get the value.
	\param [in, out] validity The validity of the returned value is intersected into the input value of this parameter.
	...
	*/
	...
	\note The ParamType must be one of the following, otherwise no get occurs and a value of Point3::Origin returned:
	TYPE_POINT3, TYPE_RGBA, TYPE_HSV
	\note Starting in 3ds Max 2016, please use \ref IParamBlock2::GetPoint3(ParamID,TimeValue,Interval&,int) instead; it supports a validity interval
	and is more robust in case of invalid parameter IDs.
	*/
	...
	*/
	...
	TYPE_POINT4, TYPE_FRGBA
	\note Starting in 3ds Max 2016, please use \ref IParamBlock2::GetPoint4(ParamID,TimeValue,Interval&,int) instead; it supports a validity interval
	and is more robust in case of invalid parameter IDs.
	*/
	...
	*/
	...
	TYPE_INT, TYPE_BOOL, TYPE_TIMEVALUE, TYPE_RADIOBTN_INDEX, TYPE_INDEX
	\note Starting in 3ds Max 2016, please use \ref IParamBlock2::GetInt(ParamID,TimeValue,Interval&,int) instead; it supports a validity interval
	and is more robust in case of invalid parameter IDs.
	*/
	...
	*/
	...
	TYPE_FLOAT, TYPE_ANGLE, TYPE_PCNT_FRAC, TYPE_WORLD, TYPE_COLOR_CHANNEL
	\note Starting in 3ds Max 2016, please use \ref IParamBlock2::GetFloat(ParamID,TimeValue,Interval&,int) instead; it supports a validity interval
	and is more robust in case of invalid parameter IDs.
	*/
	...
	*/
	...
	\note The ParamType must be one of the following, otherwise no get occurs and a value of 0 will be returned:
	TYPE_INT, TYPE_BOOL, TYPE_TIMEVALUE, TYPE_RADIOBTN_INDEX, TYPE_INDEX
	\note Starting in 3ds Max 2016, please use \ref IParamBlock2::GetTimeValue(ParamID,TimeValue,Interval&,int) instead; it supports a validity interval
	and is more robust in case of invalid parameter IDs.
	*/
	...
	*/
	...
	TYPE_FILENAME or TYPE_STRING
	\note Starting in 3ds Max 2016, please use \ref IParamBlock2::GetStr(ParamID,TimeValue,Interval&,int) instead; it supports a validity interval
	and is more robust in case of invalid parameter IDs.
	*/
	...
	TYPE_FILENAME or TYPE_STRING
	*/
	const MCHAR* GetStr(const ParamID id, const TimeValue t, Interval& validity, const int tabIndex = 0);
	...
	\note The ParamType must be one of the following, otherwise no get occurs and a value of NULL will be returned:
	TYPE_MTL
	\note Starting in 3ds Max 2016, please use \ref IParamBlock2::GetMtl(ParamID,TimeValue,Interval&,int) instead; it supports a validity interval
	and is more robust in case of invalid parameter IDs.
	*/
	...
	TYPE_MTL
	*/
	Mtl* GetMtl(const ParamID id, const TimeValue t, Interval& validity, const int tabIndex = 0);
	...
	\note The ParamType must be one of the following, otherwise no get occurs and a value of NULL will be returned:
	TYPE_TEXMAP
	\note Starting in 3ds Max 2016, please use \ref IParamBlock2::GetTexmap(ParamID,TimeValue,Interval&,int) instead; it supports a validity interval
	and is more robust in case of invalid parameter IDs.
	*/
	...
	TYPE_TEXMAP
	*/
	Texmap* GetTexmap(const ParamID id, const TimeValue t, Interval& validity, const int tabIndex = 0);
	...
	\note The ParamType must be one of the following, otherwise no get occurs and a value of NULL will be returned:
	TYPE_BITMAP
	\note Starting in 3ds Max 2016, please use \ref IParamBlock2::GetBitmap(ParamID,TimeValue,Interval&,int) instead; it supports a validity interval
	and is more robust in case of invalid parameter IDs.
	*/
	...
	TYPE_BITMAP
	*/
	PBBitmap* GetBitmap(const ParamID id, const TimeValue t, Interval& validity, const int tabIndex = 0);
	...
	\note The ParamType must be one of the following, otherwise no get occurs and a value of NULL will be returned:
	TYPE_INODE
	\note Starting in 3ds Max 2016, please use \ref IParamBlock2::GetINode(ParamID,TimeValue,Interval&,int) instead; it supports a validity interval
	and is more robust in case of invalid parameter IDs.
	*/
	...
	TYPE_INODE
	*/
	INode* GetINode(const ParamID id, const TimeValue t, Interval& validity, const int tabIndex = 0);
	...
	\note The ParamType must be one of the following, otherwise no get occurs and a value of NULL will be returned:
	TYPE_MTL, TYPE_TEXMAP, TYPE_INODE, TYPE_REFTARG, TYPE_PBLOCK2, TYPE_OBJECT, TYPE_CONTROL
	\note Starting in 3ds Max 2016, please use \ref IParamBlock2::GetReferenceTarget(ParamID,TimeValue,Interval&,int) instead; it supports a validity interval
	and is more robust in case of invalid parameter IDs.
	*/
	...
	TYPE_MTL, TYPE_TEXMAP, TYPE_INODE, TYPE_REFTARG, TYPE_PBLOCK2, TYPE_OBJECT, TYPE_CONTROL
	*/
	ReferenceTarget* GetReferenceTarget(const ParamID id, const TimeValue t, Interval& validity, const int tabIndex = 0);
	...
	\note The ParamType must be one of the following, otherwise no get occurs and a value of NULL will be returned:
	TYPE_PBLOCK2
	\note Starting in 3ds Max 2016, please use \ref IParamBlock2::GetParamBlock2(ParamID,TimeValue,Interval&,int) instead; it supports a validity interval
	and is more robust in case of invalid parameter IDs.
	*/
	...
	TYPE_PBLOCK2
	*/
	IParamBlock2* GetParamBlock2(const ParamID id, const TimeValue t, Interval& validity, const int tabIndex = 0);
	...
	TYPE_MATRIX3
	\note TYPE_MATRIX3 items cannot be animated.
	\note Starting in 3ds Max 2016, please use \ref IParamBlock2::GetMatrix3(ParamID,TimeValue,Interval&,int) instead; it supports a validity interval
	and is more robust in case of invalid parameter IDs.
	*/
	...
	\param id The permanent ID of the parameter.
	\param t The time at which to get the value.
	\param[inout] validity The validity of the returned value is intersected into the input value of this parameter.
	\param tabIndex If the parameter is a Tab\<\> this is the zero based index into the
	table of the value to get.
	\return  The Matrix3 value retrieved.
	\note The ParamType must be one of the following, otherwise no get occurs and a value of IdentityMatrix will be returned:
	...
	*/
	...
	TYPE_FILENAME
	\note Starting in 3ds Max 2016, please use \ref IParamBlock2::GetAssetUser(ParamID,TimeValue,Interval&,int) instead; it supports a validity interval
	and is more robust in case of invalid parameter IDs.
	*/
...
//! @{	
	...
	/*! \remarks Insert <b>num</b> Point2* \\ Point3* \\ Point4* elements at position <b>at</b>.
	\param id The permanent ID of the parameter.
	\param at Zero based array index where to insert the elements.
	\param num Number of elements to insert.
	\param el Array of elements to insert.
...
//! @{
	...
	/*! \remarks Append <b>num</b> Point2* \\ Point3* \\ Point4* elements at the end of the
	array.
	\param id The permanent ID of the parameter.
	\param num The number of elements to append to the end of the array.
	\param el The elements to append.
...
//! \name Anim Num/Param ID Conversion
//! @{	
...
//! \name Parameter Dimension Related Methods
//! @{	
	...
	\param paramNum The index into the parameter block of the parameter to rescale.
	\param tabIndex If the parameter is a Tab\<\> this is the zero based index into the 
...
//! \name Parameter Change Notification Methods
//! @{	
...
//! @{
	...
	materials editor). See Class IAutoMParamDlg. */
...
//! \name MAXScript Default Parameter Initialization
//! @{	
...
//! @{	
	...
	describes the parameter alias whose name is passed. This includes the
	name, ParamID and Tab\<\> index.
	\param alias_name The name of the alias to find. */
	virtual ParamAlias* FindParamAlias(const MCHAR* alias_name)=0;
	/*! \remarks Finds the name of a parameter alias using the ID and
	Tab\<\> index passed.
	\param id The permanent ID of the parameter.
	\param tabIndex If the parameter is a Tab\<\> this is the zero based index into the
	table. If not a Tab\<\> use the default of -1.
	\return  The name of the alias or NULL if not found. */
	virtual const MCHAR* FindParamAlias(ParamID id, int tabIndex=-1)=0;
...
//! \name SubAnim Numbering Related Methods
//! @{	
...
//! \name Copying Parameter Values Between Blocks
//! @{	
...
//! @{
	...
	the reset.
	\param updateUI  TRUE to update the user interface; FALSE to not update.
	...
	\param updateUI  Determines if the user interface is updated for the parameter. TRUE to
...
//! \name PBAccessor Get/Set Methods
//! @{	
...
//! \name Validity of Parameter Block
//! @{	
...
//! @}
	...
	/*! Set a parameter value using the parameter name. This method finds the parameter with the matching internal name
	(see ParamDef::int_name) and calls SetValue() on that parameter. The method asserts and returns false if the 
	parameter is not found. Otherwise, it returns the result of the call to SetValue().
	\remarks The name comparison is case-insensitive.
	\remarks This method performs a linear search over the parameters in the parameter block; it may therefore be expensive to call.
	\param paramName The internal name of the parameter to be set (see ParamDef::int_name)
	\param value The value to be set
	\param t The time at which to set the value
	\param tabIndex The index of the value to set, if the parameter is of TAB type. */
	template<typename T>
	bool SetValueByName(const MCHAR* const paramName, const T& value, const TimeValue t, const int tabIndex = 0);

	/*! Get a parameter value using the parameter name. This method finds the parameter with the matching internal name
	(see ParamDef::int_name) and calls GetValue() on that parameter. The method asserts and returns false if the 
	parameter is not found. Otherwise, it returns the result of the call to GetValue().
	\remarks The name comparison is case-insensitive.
	\remarks This method performs a linear search over the parameters in the parameter block; it may therefore be expensive to call.
	\param paramName The internal name of the parameter to get (see ParamDef::int_name)
	\param value The value to get
	\param[out] validity The validity of the returned value is intersected into the input value of this parameter.
	\param t The time at which to get the value
	\param tabIndex The index of the value to get, if the parameter is of TAB type. */
	template<typename T>
	bool GetValueByName(const MCHAR* const paramName, const TimeValue t, T& value, Interval& validity, const int tabIndex = 0);

        //! \name Parameter Iteration
        //! \brief Standard, STL-like iteration on the parameters stored in this block.
        ...
        /*! Returns an iterator to the first parameter in the block. 
            ...
            \remark If there are no parameters, then begin() == end().
            \see end() */
        ParameterIterator begin();
        /*! Returns an iterator that is one past the last valid parameter. 
        \remark If there are no parameters, then end() == begin().
        \see begin() */
...
// the Validate() function should return false if the given PB2Value is
// not valid
/*! \sa  Structure ParamDef, ::ParamType, ::ParamType2, 
Class ParamBlockDesc2.\n\n
\par Description:
...
public:
	// get into v
	/*! \remarks This method is called when IParamBlock2::GetValue is called on parameter 'id', 
	and the parameter value is v. This method is called after the controller is
	accessed, so the current controller value is seen and can be optionally
	overridden in this method. Note that the controller is accessed
	...
	<b>{}</b> 
	Notes:
	The content of 'v' is the value from the IParamBlock2 parameter, and can be changed by this Get method.
	If the parameter type is such that the parameter value is a data value held by a pointer (v.p, v.p4, v.m, v.p2)
	that pointer is guaranteed to be non-null, and the Get method must not set it to null. Either the existing data 
	value must be updated in-place (pointer unchanged), or the existing data value must be deleted (typically using 
	PB2Value::Free) and a new pointer assigned. The PB2Value will free this pointer when it is done using the data.
	If the parameter type is TYPE_STRING or TYPE_FILENAME, v.s contains a pointer to a copy of
	the string, and owns this copy. v.s may be null. The string can be modified in place, or can be replaced with
	a new string using the PB2Value's ReplaceString method.  The PB2Value will free this pointer when it is done using 
	the data.
	If the parameter type is TYPE_BITMAP, v.bm contains a pointer to a PBBitmap, and owns this pointer. v.bm may be null. 
	The PBBitmap can be modified in place, or the existing PBBitmap must be deleted (typically using PB2Value::Free) 
	and a new pointer assigned (typically by cloning a PBBitmap. The PB2Value will free this pointer when it is done 
	using the data.
	*/
	virtual void Get(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t, Interval &valid) { }    

	// set from v
	/*! \remarks This method is called when IParamBlock2::SetValue is called on parameter 'id', and the parameter 
	value being set is v. This is called just before calling <b>SetValue()</b> on the parameter's
	...
	<b>{}</b> 
	Notes: 
	The content of 'v' is the value being set on the IParamBlock2 parameter, and can be changed by this Set method.
	If you get the value of the parameter from the parameter block from within the Set method, whether the value returned 
	will be the existing value or the new value varies by parameter type and whether the parameter is animatable. 
	For parameter types that correspond to data types such as float, integer, and point3, the Set method is called before 
	the parameter block parameter is set unless the parameter block parameter is already animated or setting the value 
	would cause it to be animated. If the parameter block parameter would remain constant after the set, the Set 
	method is called after the parameter block parameter is set.
	If the parameter type is such that the parameter value is a data value held by a pointer (v.p, v.p4, v.m, v.p2)
	that pointer is guaranteed to be non-null, and the Set method must not set it to null. Either the existing data
	value must be updated in-place (pointer unchanged), or the existing data value must be deleted (typically using
	PB2Value::Free) and a new pointer assigned. The PB2Value will free this pointer when it is done using the data.
	If the parameter type is TYPE_STRING or TYPE_FILENAME, v.s contains a pointer to a copy of
	the string, and owns this copy. v.s may be null. The string can be modified in place, or can be replaced with
	...
	*/
	...
	// PreSet from v
	/*! \remarks This method is called when IParamBlock2::SetValue is called on parameter 'id', and the parameter
	value being set is v. This is called before setting the parameter's value or calling <b>SetValue()</b> 
	on the parameter's controller, so it can take note of the value going in and change it if desired.
	\par Parameters:
	<b>PB2Value\& v</b>\n\n
	The value being set.\n\n
	<b>ReferenceMaker* owner</b>\n\n
	...
	<b>{}</b> 
	Notes: 
	The content of 'v' is the value being set, and can be changed by the PreSet method.
	If you get the value of the parameter from the parameter block from within the PreSet method, the value
	returned will be the existing value.
	If the parameter type is such that the parameter value is a data value held by a pointer (v.p, v.p4, v.m, v.p2)
	that pointer is guaranteed to be non-null, and the PreSet method must not set it to null. Either the existing data
	value must be updated in-place (pointer unchanged), or the existing data value must be deleted (typically using
	PB2Value::Free) and a new pointer assigned. The PB2Value will free this pointer when it is done using the data.
	If the parameter type is TYPE_STRING or TYPE_FILENAME, v.s contains a pointer to a copy of
	the string, and owns this copy. v.s may be null. The string can be modified in place, or can be replaced with
	...
	*/
	...
	// PostSet from v
	/*! \remarks This method is called when IParamBlock2::SetValue is called on parameter 'id', and the parameter
	value being set is v. This is called after setting the parameter's value or calling <b>SetValue()</b>
	on the parameter's controller.
	\par Parameters:
	<b>const PB2Value\& v</b>\n\n
	...
	<b>{}</b> 
	Notes: 
	The content of 'v' is the value that was set, and cannot be changed by the PostSet method.
	If you get the value of the parameter from the parameter block from within the PostSet method, the value
	returned will be the new value.
	If the parameter type is such that the parameter value is a data value held by a pointer (v.p, v.p4, v.m, v.p2)
	that pointer is guaranteed to be non-null.
	*/
	...
	<b>P_COMPUTED_NAME</b> parameter flag, you also need to supply a
...
/** This interface should be implemented by PBAccessor-derived classes if it needs to respond differently to Set calls
when the system is performing rescaling of parameter block parameters. See the EggShape sample plugin for an example. */

#define PB_RESCALE_PARAM_ACCESSOR_INTERFACE Interface_ID(0x195f6f32, 0x5f2126d0)

class PBRescaleParamAccessor : public BaseInterface
{
public:
	...
	controller, so it can take note of the value going in and change it if desired.
	NOTE: This is ONLY called when the Set is being performed as part of a RescaleParam
	operation, such as happens when RescaleWorldUnits() is called.
	NOTE: This method is NOT called when the parameter in question is animated, as the
	system rescales the controller separately.
	\par Parameters:
	<b>PB2Value\& v</b>\n\n
	The value being set (already scaled).\n\n
	<b>float scaleFactor</b>\n\n
	The scale factor that was used to compute the new value (above).\n\n
	<b>ReferenceMaker* owner</b>\n\n
	Points to the owner of the parameter block.\n\n
	<b>ParamID id</b>\n\n
	The permanent ID of the parameter.\n\n
	...
	\par Default Implementation:
	<b>{}</b> */
	virtual void Set(PB2Value& v, float scaleFactor, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t) { }						 
};
...
/*! \sa Class ParamBlockDesc2, Structure PB2Value, Class PBValidator, Class PBBitmap, ::ParamType, ::ParamType2, 
...
struct ParamDef: public MaxHeapOperators
{
	...
	const MCHAR*		int_name;		// fixed internal name
	/*! This is the type of the parameter. See ::ParamType2.  */
	...
	short		ref_no;			// block-owner's reference number for non-hosted ReferenceTargets
	...
	Class_ID	class_ID;		// validator for reference targets
	...
	*/
	...
	ParamID*	enable_ctrls;	// array of which other params have their UI controls automatically enabled by this param
	...
	int		caption;		// caption string res ID for open/save file dialogs
	/*! The ToolTip string res ID.  */
	int	toolTip;
	/*! The initial filename for open/save file dialogs.  */
	const MCHAR*		init_file;		// initial filename for open/save file dialogs
	/*! The file types string resource ID for open/save file dialogs (in MAXScript type: form)  */
	int		file_types;		// file types string res ID for open/save file dialogs (in MAXScript type: form)
	// new for R4
	...
	A parameter is considered an asset if it's type if TYPE_FILENAME or TYPE_FILENAME_TAB and 
	is flagged with P_HAS_ASSETTYPE or P_HAS_ASSETTYPENAME. */
...
private:
	...
	IAutoRendererParamDlg*	masterRDlg;		// master render dialog if any
...
public:
	...
	*/
	...
	implementation of the method used by plug-ins using the ParamBlock2
	system.\n\n
	Returns true if the proc was registered with a parameter map. If the proc
	was not registered with the parameter map, DeleteThis() will not 
	be called on proc when the user dialog procs are cleared, potentially
	...
	inline bool SetUserDlgProc(ParamBlockDesc2* pbd, ParamMap2UserDlgProc* proc=NULL) { return SetUserDlgProc(pbd, 0, proc); }
    ...
    //! \brief Creates the automatically-managed parameter dialogs defined with the P_AUTO_UI or P_AUTO_UI_QT flags in the parameter block descriptors.
    /*! This method should generally be called from Renderer::CreateParamDialog().
        ...
        \param renderer The renderer for which the dialogs are being created.
    ...
    //! \sa IAutoRendererParamDlg* CreateParamDialogs(IRendParams *ip, Renderer& renderer)     
...
public:
    ...
    //! \brief Creates a Qt widget to be associated with the given parameter map ID.
    /*! This method is to be implemented by the plugin developer, if and only if the P_AUTO_UI_QT flag has been specific on a parameter block descriptor.
        It replaces the UI definition that exists in the parameter block descriptor for Win32 dialogs; it is responsible for building a Qt widget
        which defines the UI to be controlled by the parameter map with the given ID. This Qt widget will be hosted in a rollup window.
        \param owner The owner of the dialog. This will normally be the owner of the parameter block, but in the case of P_CLASS_PARAMS, where the parameter block doesn't
            have an owner, this will be the object for which the dialog is being displayed.
        \param paramBlock The parameter block to which the parameter map belongs.
        \param paramMapID The ID of the parameter map, as specific in the parameter block descriptor.
        \param [out] rollupTitle The title to be displayed in the rollup's header.
        \param [out] rollupFlags Optional flags, such as APPENDROLL_CLOSED. The default input value is 0.
        \param [out] rollupCategory Optional value controlling the order in which rollups are positioned. The default input value is ROLLUP_CAT_STANDARD.
        \param [out] tabID Optional identifier for the tab in which the rollup is to be added. This is the ID used to create new tabs
            in implementations of ITabDialogObject::AddTabToDialog(). Note that tabs are currently only supported in the render settings
            dialog. The default input value is ClassID(); this value causes the widget to be added to the default/first tab.        \return An instance of a new Qt widget which will be hosted in the parameter map's rollup. 
        \remark <b>The ownership of the Qt widget is transferred to the caller.</b>
        \remark May return null if no dialog should be shown for the given parameter map.
        \sa \ref parammap_qt. */
	virtual MaxSDK::QMaxParamBlockWidget* CreateQtWidget(
		ReferenceMaker& owner,
		IParamBlock2& paramBlock,
...
Vanilla ClassDesc2 manages only modal ParamMaps, and those are destroyed when EndEditParams is called.
This extension allows plugins to support modeless user interface rollups that are tied to parameter blocks.
...
class ModelessParamMapClassDesc2 : public ClassDesc2
{
protected:
	Tab<IParamMap2*> modelessParamMaps;
public:
	/*! \remarks Destructor. */
	PB2Export virtual ~ModelessParamMapClassDesc2();
	/*! \remarks Implemented by the System.\n\n
	Returns the number of modeless parameter map2s, which are maintained by the class. */
	virtual int			NumModelessParamMaps() { return modelessParamMaps.Count(); }
	/*! \remarks Implemented by the System.\n\n
	Gets the modeless parameter map indicated by the index.\n\n
	\param i The index of the desired modeless parameter map.
	\return A pointer to the modeless parameter map2 requested.
	*/
	virtual IParamMap2*	GetModelessParamMap(int i) { return modelessParamMaps[i]; }
	/*! \remarks Implemented by the System.\n\n
	Returns a pointer to the parameter map2 as specified by the parameter
	block2 pointer and optional map ID index passed.
	\param pbd Points to the parameter block descriptor2 associated with this parameter map.
	\param map_id This parameter specifies the ID of the map/rollout to get. */
	PB2Export virtual IParamMap2*	GetModelessParamMap(ParamBlockDesc2* pbd, MapID map_id = 0);
	/*! \remarks Implemented by the System.\n\n
	Adds a parameter map to the list of modeless maps being managed.
	\param map Points to the parameter map2 to add to the list. */
	PB2Export virtual void AddModelessParamMap2(IParamMap2* map);
	/*! \remarks Implemented by the System.\n\n
	Removes a parameter map from the list of modeless maps being managed.
	\param map Points to the parameter map2 to remove from the list. */
	PB2Export virtual void RemoveModelessParamMap2(IParamMap2* map);
	/*! \remarks Implemented by the System.\n\n
	Removes a parameter map from the list of modeless maps being managed using its id.
	\param id Provides the id of the map to remove from the list. */
	PB2Export virtual void RemoveModelessParamMap2(int id);
	/*! \remarks Implemented by the System.\n\n
	//! \brief Creates a Qt widget to be associated with the given parameter map ID for modeless use.
	/*! This method is to be implemented by the plugin developer, if and only if the P_AUTO_UI_QT flag has been specific on a parameter block descriptor.
	It replaces the UI definition that exists in the parameter block descriptor for Win32 dialogs; it is responsible for building a Qt widget
	which defines the UI to be controlled by the parameter map with the given ID. This Qt widget will be hosted in a rollup window.
	\param owner The owner of the dialog. This will normally be the owner of the parameter block, but in the case of P_CLASS_PARAMS, where the parameter block doesn't
	...
	\remark May return null if no dialog should be shown for the given parameter map.
	\sa \ref parammap_qt. */
	PB2Export virtual MaxSDK::QMaxParamBlockWidget* CreateModelessQtWidget(ReferenceMaker& owner, IParamBlock2& paramBlock, const MapID paramMapID);
	/*! \remarks Implemented by the System.\n\n
	Destroys any parameter map associated with the given parameter block and ID.
	\param pblock Gives the parameter block of the map to be destroyed
	\param id Gives the unique ID of the map to be destroyed
	\remark This only destroys the parameter map associated with the ID, not the Qt widget. */
	PB2Export virtual void DestroyModelessUI(IParamBlock2* pblock, int id);
};

// use the constructors to build both static & dynamic descriptors
/*! \sa  Class ClassDesc2, Class ParamMap2UserDlgProc, Class Mtl,  Class Texmap,  Class PBBitmap, Class PBAccessor, Class INode,  Class Color,  Class Point3, Class ReferenceTarget, Structure ParamDef, \n
...
<b><b>P_AUTO_CONSTRUCT</b></b>   <b><b>P_AUTO_UI</b></b> <b><b>P_AUTO_UI_QT</b></b>
...
private:
	...
	va_list scan_option(va_list ap, int tag, ParamDef* p, const MCHAR* parm_name, int& optionnum);
...
public:
	...
	const MSTR      int_name;      // fixed internal name
...
public:
    ...
    - P_AUTO_UI_QT \n
    Indicates this block supports automatic UI rollout management (using QT) in calls to ClassDesc2::BeginEditParams(), 
    ClassDesc2::EndEditParams(),ClassDesc2::CreateParamDlg(), ClassDesc2::CreateParamDialog(), etc. \n
    <b>No</b> arguments must be supplied to supplement this flag (unlike P_AUTO_UI which requires a dialog ID, etc.).
	- P_HASCATEGORY \n
	...
	changes to the established descriptor's parameter definitions are not reflected in the 
	...
	p_default, FALSE,
	p_ui, geo_map_2, TYPE_SINGLECHECKBOX, IDC_HEMI,
	end,
	...
	constructor arguments to indicate multiple parameter maps present. If P_AUTO_UI or P_AUTO_UI_QT is specified, 
	the usual single rollup template spec is replaced by a count (of rollups) followed 
	by that many sets of rollup specs (which are empty for P_AUTO_UI_QT), each beginning with the associated mapID. The auto 
	...
	and the segments (segs) and radius spinners in the first. \n\n
	...
	- int ref_no \n
	This is the same number that the plug-in would use to get and set the parameter block reference 
    ...
    \par [\<auto_ui_parammap_specs>]
	    ...
	    whether the ParamMap2 should be created/deleted on this call. All the bits in the supplied 
	...
	entry. If you specify this parameter flag, you also need to supply a p_accessor 
	...
	Note that P_OWNERS_REF and P_SUBTEX are both primarily intended for use when re-coding 
	...
	<b>P_READ_SECOND_FLAG_VALUE</b> \n
	Indicates that a second DWORD sized flag value follows the first in the parameter's definition.
	The second flag will be read in, shifted left DWORD*8 bits and combined with the first flag value to form the ParamDef flag value.\n\n
	<b>Second Flag Values</b>\n\n
	<b>P_USE_ACCESSOR_ONLY</b>\n
	When set, 3ds Max will get or set the value of the parameter this flag is applied to only via the accessor function, if available.\n\n
	<b>P_USE_NODE_TM_VALIDITY</b>\n
	When set, the parameter block's validity interval is intersected with that of the Node parameter's transform. Applies only to TYPE_INODE and TYPE_INODE_TAB parameters.\n\n
	<b>P_USE_NODE_OS_VALIDITY</b>\n
	When set, the parameter block's validity interval is intersected with that of the Node parameter's object state (including the world space modifiers). 
	Applies only to TYPE_INODE and TYPE_INODE_TAB parameters.\n\n
	<b>P_NO_MACRO_RECORDER</b>\n
	When set, changes to the parameter will not trigger output to the macro recorder.\n\n
	- int local_name_res_ID \n
	...
	For the possible tags and the specification arguments see \ref ParamTagsList "List of ParamTags Choices".
	*/
	...
	*/
	...
	/*! Finds the parameter with the matching internal name (ParamDef::int_name), and returns its index. Returns -1 is the parameter is not found.
	\remarks The name comparison is case-insensitive.
	\param name The internal name of the parameter to find (see ParamDef::int_name) */
	...
	/*! \remarks This method is used with static class parameter blocks
	only.\n\n
	Sets the Point2 \\ Point3 \\ Point4 value of the specified parameter at the specified time.
	\par Parameters:
	<b>ParamID id</b>\n\n
	The permanent ID of the parameter.\n\n
	<b>TimeValue t</b>\n\n
	...
	of the value to set.
	\return  TRUE on success; otherwise FALSE. */
	BOOL SetValue(ParamID id, TimeValue t, Point2& v, int tabIndex = 0) { return class_params->SetValue(id, t, v, tabIndex); }
	...
	/*! \remarks This method is used with static class parameter blocks
	only.\n\n
	Retrieves the Point2 \\ Point3 \\ Point4 value of the specified parameter at the specified
	time.
	\par Parameters:
	<b>ParamID id</b>\n\n
	The permanent ID of the parameter.\n\n
	...
	of the value to get.
	\return  TRUE on success; otherwise FALSE. */
	BOOL GetValue(ParamID id, TimeValue t, Point2& v, Interval &ivalid, int tabIndex=0) { return class_params->GetValue(id, t, v, ivalid, tabIndex); }
	...
	If the parameter is a Tab\<\> this is the zero based index into the table
	of the value to get.
	\return  The Point2 \\ Point3 \\ Point4 value of the parameter */
	...
	<b>StringResID id</b>\n\n
	The permanent ID of the parameter. */
	PB2Export const MCHAR* GetString(StringResID id) { return cd->GetRsrcString(id); }
	...
	\param fileType A string containing a correctly formatted filter list.  This buffer is maintained by the calling code
	*/
...
special case that if the ParamBlock parameter type is TYPE_RGBA, the ParamBlock2
parameter type can be either TYPE_RGBA or TYPE_FRGBA.
This override of the method fills in a Tab<DWORD>& with the subAnim renumbering
when doing the pb1 to pb2 conversion. When replacing a pb1 with a pb2, all 
instances of the pb1 should be replaced with the pb2, as objects such as 
scripted controllers may hold a pointer to the pb1 to support subAnims
(refmaker pointer/subanim number data pairs). After replacing all pb1 instances,
a REFMSG_SUBANIM_NUMBER_CHANGED NotifyDependents call should be made on the pb2
...
<b>ParamBlockDesc2* pdescNew</b>\n\n
Points to the new parameter block 2 descriptor.\n\n
<b>Tab<DWORD>& subAnimRenumbering</b>\n\n
...
\sa ProcessPB2ToPB1SaveToPrevious
*/
PB2Export int CopyParamBlock2ToParamBlock(
	IParamBlock2* pb2,
	IParamBlock *pb1, ParamBlockDescID *pdescPB1, int pb1Count);

//! \brief Helper function for handling Save To Previous for a ParamBlock to ParamBlock2 migration
/*! This method does all the required handling to support the Save To Previous handling of a ParamBlock to ParamBlock2 migration.
The code creates a ParamBlock from a ParamBlock2 and registers the ParamBlock to be saved instead of the ParamBlock2.
\param  pb2_owner The owner of the ParamBlock2 instance.
\param  pb2 The ParamBlock2 instance to process.
\param  pb2_refno The owner's reference number of the ParamBlock2 instance.
\param  pdesc An array of parameter block descriptors.
\param  count The number of parameter block descriptors in the array.
\param  version The version number of the parameter block
The following is an example usage, where all references to the ParamBlock2 instance are replaced with a ParamBlock instance.
/code
bool BoxObject::SpecifySaveReferences(ReferenceSaveManager& referenceSaveManager)
{
	// if saving to previous version that used pb1 instead of pb2...
...
public:
	...
	//! \brief This method allows for future extension.
...
/*! \remarks This function determines whether a param type is a type of reftarg. */
...
//
//! \brief Provides standard C++ iteration functionality for the parameters stored by an IParamBlock2.
/*! \par Iterator Validity
        The iterator remains valid if parameters are either added or removed from the parameter block, but will continue referring to the
        same parameter index, and therefore may fall out of bounds in the process.
    \see IParamBlock2::begin(), IParamBlock2::end() 
*/
class IParamBlock2::ParameterIterator
{
public:
...
private:
    ...
    // Never null (always initialized from a reference)
    IParamBlock2* m_param_block;
    unsigned int m_num_params;       // make sure this stays declared before m_parameter_index, because of initialization order
