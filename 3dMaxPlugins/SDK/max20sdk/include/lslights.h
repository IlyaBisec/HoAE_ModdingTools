//*****************************************************************************/
// Copyright (c) 1997,1998 Autodesk, Inc.
//
...
//! \brief Common Photometric light
...
//    for B24 of VIZ R4
	...
	/*! \param[in] name - The type name the light is to be changed to. May not be NULL.
		...
		type to an invalid value, nothing is changed, and no error is returned. */
	...
	/*! The use of this functions varies based on the current light type.
		...
		the light area is set to the triangle or quadrilateral defined by the 3 or 4 points,
		respectively. If fewer than 3 points is specified, no change is made. If more than
		4 points is specified a quadrilateral is made from the first 4 points.
	...
	/*! \param[out] pointsOut - The buffer that will receive the vertices in the shape. If bufSize is
		...
		\return The total number of vertices in the shape. This is independent of the
	...
	//! \brief Returns the color preset of the light
	/*! Color preset is used when Kelvin temperature is not used. See LightscapeLight::GetUseKelvin().
		\return The current preset color. */
	virtual PresetLightColor GetColorPreset() const = 0;
	//! \brief Sets the preset color of the light to be used
	/*! Color preset is used when Kelvin temperature is not used. See LightscapeLight::GetUseKelvin().
		\param[in] presetID - The time at which the color is set. */
...
//! \brief Extension to Photometric light
//! This interface extends class LightscapeLight with new functionality in 3ds Max 2016.
class LightscapeLight2 : public LightscapeLight 
{
public:

    //! \brief Bakes a web distribution into a spherical map.
    /*! Bakes the web (IES) distribution into a spherical map of floating-point intensities which are to be multiplied against the light's
    global intensity value. The resulting texture has the x coordinates covering the horizontal angles, from -PI to PI radians, and the y coordinates
    covering the vertical angles, from 0 to PI radians, 0 being +Z and PI being -Z. The rotation of the web distribution, returned by GetWebRotateX()
    and friends, is baked in the result. The resolution of the baked texture is determined internally and is based on the resolution of the original
    data from the web distribution (.ies) file.

    \param[out] bakedDistribution - An array of floating-point intensities which is allocated and filled by the method. The array represents a spherical
        texture map. The size of the array is: bakedWidth * bakedHeight.
    \param[out] bakedWidth - The width of the baked spherical texture map.
    \param[out] bakedHeight - The height of the baked spherical texture map.

    \return false if and only the method is invalid on this light (i.e. if the light doesn't use a web distribution). If the light uses
        a web distribution but the file could not be loaded, this method returns true with an empty baked map (the resolution is zero).
    */
    virtual bool BakeWebDistribution(MaxSDK::Array<float>& bakedDistribution, size_t& bakedWidth, size_t& bakedHeight) = 0;
