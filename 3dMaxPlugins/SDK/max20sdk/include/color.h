...
//	operator DWORD() { return RGB(FLto255(r),FLto255(g), FLto255(b)); }
    ...
    //! \name Kelvin temperature, black body
    //@{
    /*! Calculates the chromacity of a black body radiator at a given temperature in kelvins.
        \param kelvin_temperature The black body temperature in kelvins.
        \param intensity The intensity of the color to be generated (the luminance Y of the CIE XYZ color from which the RGB color is converted).
        \return An RGB value, converted from a CIE XYZ color with the calculated chromacity and provided intensity.
        \remark The returned color components is guaranteed to have positive values, but may exceed 1.0. */
    static GEOMEXPORT Color FromKelvinTemperature(const float kelvin_temperature, const float intensity = 1.0f);
    /*! Finds the kelvin temperature which, when converted to RGB, would generate the closest possible color to the current RGB value.
        For a color that was generated from a kelvin temperature, the returned value should match that temperature exactly. But for an
        arbitrary color, the returned value will only be an approximation (and may vary wildly if the color is very far from the set of valid
        black body colors).
        \return The temperature, in kelvins, which would generate the closest matching color to 'this'.
        \remark This performs a search through the range of valid color temperatures, and may therefore be a bit expensive. */
    GEOMEXPORT float FindClosestMatchingKelvinTemperature() const;
    //@}
};
