...
class URLTab : public Tab<const MCHAR*>
{
public:
...
/*! \brief Manager class for Drag and Drop
	...
	\note In Max 2011, the management of DragAndDropHandler memory lifetime was 
	changed. In previous versions, it was up to 3rd party developers to 'unregister' 
...
public:
	...
	virtual const MCHAR* GetDownloadDirectory()=0;
...
public:
	...
	\par Parameters:
	<b>HWND window</b>\n\n
	The specified handle to the window in which the drag and drop event is occurring. This
	is one of the windows that was enabled via a
	<b>IDragAndDropMgr::EnabledDnD()</b> call, so it may be the parent of the
	lowest-level window that the mouse is actually over.\n\n
	<b>DropType* type</b>\n\n
...
protected:
   ...
   CoreExport static bool loaded;            // flags if current package already downloaded
...
#define MCGFILE_DROPTYPE        0x0000000b		//!< The MCG type.
//@}
...
protected:
	...
	CoreExport static MSTR drag_setting;

   // URL download utilities
   CoreExport static bool CheckForCachedFile(const MCHAR* filename);
   CoreExport static bool IsInternetCachedFile(const MCHAR* filename);
   CoreExport static bool  AppendUrlFilename(const MCHAR* szUrl, MSTR& pathname, bool& is_URL);
...
public:
   ...
   static void Init() { drag_setting = _T(""); current_package.Clear(); DropType::Init(); }
   ...
   CoreExport static FileDropType* FindDropType(const MCHAR* filename, IDataObject* pDataObject = NULL);
   ...
   \par Default Implementation:
   <b>{ return false; }</b> */
   virtual bool     CheckDropType(const MCHAR* filename) { return false; } 
   ...
   The download progress dialog can be displayed by passing true.
   \return  TRUE if the download was successful, otherwise FALSE. */
   CoreExport static bool   DownloadPackage(URLTab& package, const MCHAR* szDirectory, HWND hwnd = NULL, bool showProgress = true);
   ...
   after being dropped.
   \return  TRUE if the download was successful, otherwise FALSE. */
   CoreExport static bool DownloadUrlToDisk(HWND hwnd, const MCHAR* szUrl, const MCHAR* szPathname, DWORD flags=0);
...
/*! \sa  Class FileDropType,  \ref dropTypes.\n\n
\par Description:
This class represents the mcg (*.mcg) DropType and is available through
the built-in type instance <b>mcgFileDropType</b>.  */
class MCGFileDropType : public FileDropType
{
public:
	// From DropType
	/*! \remarks This method returns the typecode of the DropType.
	\par Default Implementation:
	<b>{ return MCGFILE_DROPTYPE; }</b> */
	int TypeCode() { return MCGFILE_DROPTYPE; }
...
extern CoreExport MCGFileDropType mcgConfigDropType;
