...
required methods of Animatable, ReferenceMaker, ReferenceTarget, BaseObject,
...
protected:
	...
	PolyLibExport RefResult NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate );
...
public:
	...
	// IObjectDisplay2 entry
	PolyLibExport virtual bool PrepareDisplay(
		const MaxSDK::Graphics::UpdateDisplayContext& prepareDisplayContext);

	PolyLibExport virtual bool UpdatePerNodeItems(
		const MaxSDK::Graphics::UpdateDisplayContext& updateDisplayContext,
		MaxSDK::Graphics::UpdateNodeContext& nodeContext,
		MaxSDK::Graphics::IRenderItemContainer& targetRenderItemContainer);

	PolyLibExport virtual unsigned long GetObjectDisplayRequirement() const;
	...
	PolyLibExport Interval PartValidity (ChannelMask channels);
	PolyLibExport void SetChannelValidity (int i, Interval v);
	PolyLibExport void SetPartValidity (ChannelMask channels, Interval v);
...
// must run your plugin with 3ds max version 4.2 or higher.
	...
	/*! \remarks	This does error finding and repairing on the mesh
	if the mesh topology has changed since the last call. 
	\par Parameters:
	<b>INode *inode</b>\n\n
	The node in the scene.*/
	PolyLibExport void CheckMesh(INode *inode);
	...
	const MCHAR *GetObjectName() { return _M("PolyMesh"); }
	...
	<b>bool notifyDependents</b> (default true)\n
	If true, sends REF_CHANGE message to dependents as expected. Option allows you to turn this off
	under special circumstances, e.g. on object creation/copy. */
	PolyLibExport void SetDisplacementApproxToPreset(int preset, bool notifyDependents=true);
...
PolyLibExport void MakeImmediatePolyObjectWireframeRenderItemsAsGizmo(
	MaxSDK::Graphics::IRenderItemContainer& targetRenderItemContainer);
