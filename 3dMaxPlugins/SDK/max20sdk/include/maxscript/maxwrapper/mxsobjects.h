...
#define BEZIERSHAPE_READ_ACCESS		0  // beziershape access modes
...
extern ScripterExport const MCHAR* get_deleted_obj_err_message();
...
#pragma pop_macro("new")
	...
	virtual	const MCHAR* class_name() = 0;
	virtual ReferenceTarget* get_max_object() { return (NumRefs()) ? GetReference(0) : NULL; }
	ScripterExport Value*		copy_no_undo(Value** arg_list, int count);
	BOOL		derives_from_MAXWrapper() { return TRUE; }
	Value*		deep_copy(HashTable* /* remapper */) { return this; }
	...
	virtual void GetClassName(MSTR& s) { s = _M("MAXWrapper"); } // from Animatable
...
private:
	friend void initialize_MAXScript();
	static void AppDataLoadProc(Animatable *anim, const Class_ID& cid, SClass_ID sid, ILoad* iload, Tab<DWORD> &subIDs);
};
...
// metadata vararg tags used in MAXClass va_arg processing and as the type of descriptor stored in parm_desc::desc_type
enum metadata_flag : byte
{
	accessors = 1,	// beginning of accessor section (only section type currently defined). Terminated with a 'p_end'.
	fns,				// beginning of function type accessors section. Terminated with a 'p_end'. 
	paramblock,		// beginning of paramblock type accessors section. Terminated with a 'p_end'. 
	paramblock2,	// beginning of paramblock2 type accessors section. Terminated with a 'p_end'. 
};

// flag values for parm_desc::flags
enum parm_desc_flags : byte
{
	PD_NO_INIT			= 0x01,	// flags this property as requiring no init
	PD_LOCAL_DUP		= 0x02,	// this is a duplicate local translation
	PD_HAS_LOCAL_DUPS	= 0x04,	// this has duplicate local translations
	PD_SHOWN				= 0x08	// temp flag to mark already shown props in showProps()
};
 ...
 * about creation parameters, etc. */
// parameter descriptor struct
struct parm_desc			
{
	Value*				name;				// parameter name
	metadata_flag		desc_type;		// descriptor type... paramblock/fn/etc.
	...
	union
	{
		...
		struct					// paramblock2 entry descriptor
		{
			BlockID		pb2_block_id;		// the block ID of the pb2
			ParamID		pb2_param_id;		// the parameter id in the pb2
			int			pb2_tab_index;		// the tab index
		};
	};
	ParamType	type;			// common type code & initial value
	union
	{
		float			fval;
		int			ival;
...
#	define		is_tab_param(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(MAXPB2ArrayParam))
	void		collect();
	ScripterExport void		sprin1(CharStream* s);
	const MCHAR*		class_name() { return _M("Array Parameter"); }
...
#pragma pop_macro("new")
	...
	Modifier*	find_modifier(const MCHAR* name);
	Modifier*	find_modifier(MAXClass* the_class);
	const MCHAR*		class_name();
...
public:
	...
	void		collect();
	ScripterExport void		sprin1(CharStream* s);
	const MCHAR*		class_name() { return _M("SubAnim"); }
	ScripterExport ReferenceTarget* get_max_object();
...
public:
	...
	void		collect();
	ScripterExport void		sprin1(CharStream* s);
	const MCHAR*		class_name() { return _M("MAXRefTarg"); }
...
public:
	...
	void		collect();
	ScripterExport void		sprin1(CharStream* s);
	const MCHAR*		class_name() { return _M("TrackViewNode"); }
...
private:
	MAXTVNode(ITrackViewNode* tvNode);  // for wrapping standalone instances
};
...
#	define is_mesh(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(MeshValue))
	...
	const MCHAR*		class_name() { return _M("TriMesh"); }
...
/* ---------------------- MAXBezierShapeClass ----------------------- */
// the sole instance of the MAXBezierShapeClass represents the Nodes made from the core bezier object class SplineShape.
// reflecting its special status in MAX, it is defined by a separate class in MAXScript,
// allowing access to all the things you want to get at when procedurally working 
// with bezier shapes.
 
visible_class (MAXBezierShapeClass)

class MAXBezierShapeClass : public Value
{
public:
				MAXBezierShapeClass();

				classof_methods (MAXBezierShapeClass, Value);
	void		collect();
	ScripterExport void		sprin1(CharStream* s);
	void		export_to_scripter();

	ScripterExport Value* apply(Value** arglist, int count, CallContext* cc=NULL);		// object creation by applying class object
	Class_ID	get_max_class_id() { return splineShapeClassID; } 
};

/* ---------------------- BezierShapeValue ----------------------- */
// A wrapper for the MAX SDK BezierShape objects, typically used in Shape plugins for creating scene
// shapes

applyable_class (BezierShapeValue)

class BezierShapeValue : public MAXWrapper
{
public:
	Object*		obj;		// MAX object if indirect ref to shape (ref 0)
	BezierShape*		beziershape;		// direct shape ref
	BOOL		owned;		// if direct ref owned by this value (& so s/b deleted on collect)

	ScripterExport BezierShapeValue(BezierShape* ibeziershape, BOOL owned = FALSE);
	ScripterExport BezierShapeValue(Object* obj);
	static ScripterExport Value* intern(Object* obj);
			   ~BezierShapeValue();
#pragma push_macro("new")
#undef new	
	// Needed to solve ambiguity between MaxWrapper's operators and MaxHeapOperators
	using Collectable::operator new;
	using Collectable::operator delete;
#pragma pop_macro("new")

				classof_methods (BezierShapeValue, MAXWrapper);
#	define is_beziershape(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(BezierShapeValue))
	void		collect();
	void		sprin1(CharStream* s);
	const MCHAR*		class_name() { return _M("BezierShape"); }
	ReferenceTarget* get_max_object() { return obj; }
protected:
	virtual void		SetReference(int i, RefTargetHandle rtarg);
public:
	...
	void		setup_beziershape(int access = BEZIERSHAPE_READ_ACCESS);

	// bezier shape protocol (same as for MAXNode) - TBD
	def_generic				( setBezierShape,				"setBezierShape");
	def_property( beziershape );

	BezierShape*		to_beziershape() { setup_beziershape(); return beziershape; }

	void	    to_fpvalue(FPValue& v) { setup_beziershape(); v.bezshape = beziershape; v.type = TYPE_BEZIERSHAPE; }
	
	Value*		get_property(Value** arg_list, int count);
	Value*		set_property(Value** arg_list, int count);

	BezierShape*		set_up_beziershape_access(int access, ReferenceTarget** owner = NULL);
	BezierShape*		set_up_beziershape_vertex_access(int index, int access, ReferenceTarget** owner = NULL);
};
...
public:
	...
	void		collect();
	ScripterExport void		sprin1(CharStream* s);
	const MCHAR*		class_name() { return _M("Atmospheric"); }
...
public:
	...
	void		collect();
	ScripterExport void		sprin1(CharStream* s);
	const MCHAR*		class_name() { return _M("Effect"); }
...
public:
	...
	void		collect();
	ScripterExport void		sprin1(CharStream* s);
	const MCHAR*		class_name() { return _M("MultiPassCameraEffect"); }
...
public:
	...
	void		collect();
	ScripterExport void		sprin1(CharStream* s);
	const MCHAR*		class_name() { return _M("MAXShadowType"); }
...
public:
	...
	void		collect();
	ScripterExport void		sprin1(CharStream* s);
	const MCHAR*		class_name() { return _M("MAXRenderer"); }
...
#	define		is_custattrib(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(MAXCustAttrib))
	...
	void		collect();
	ScripterExport void		sprin1(CharStream* s);
	const MCHAR*		class_name() { return _M("MAXCustAttrib"); }

//	Value*		get_property(Value** arg_list, int count);
//	Value*		set_property(Value** arg_list, int count);

	def_property   ( name );
	ScripterExport CustAttrib* to_custattrib();
};
...
#pragma pop_macro("new")
	...
	CustAttrib*	find_CustAttrib(const MCHAR* name);
...
public:
	...
	void		collect();
	void		gc_trace();
	const MCHAR*		class_name() { return _M("<ChangeHandler>"); }
...
#	define is_rootnode(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(MAXRootNode))
	void		collect();
	ScripterExport void	sprin1(CharStream* s);
	const MCHAR*		class_name() { return _M("<root>");	}
...
struct node_map
{
	...
	int			count;
	BOOL		(*selector)(INode*, int, const void*);	// set selector fn
	const void*	sel_arg;							// arg for the selector
...
struct node_find
{
	BOOL		(*tester)(INode*, int, const void*);		// node test fn
	const void*	test_arg;									// tester arg
	BOOL		(*selector)(INode*, int, const void*);		// set selector fn
	const void*	sel_arg;									// arg for the selector
};
...
extern BOOL all_objects_selector(INode* node, int level, const void* selector_arg);
...
public:
	...
	const MCHAR*			class_name() { return _M("XRefScene"); }
...
#	include "../protocols/xrefs.inl"
	...
	def_property( duplicateMatchingLayers );
...
public:
	...
	static ScripterExport Value* intern(ReferenceTarget* ref);
	ScripterExport void		sprin1(CharStream* s);
	const MCHAR*		class_name() { return _M("<rootScene>");	}
};
