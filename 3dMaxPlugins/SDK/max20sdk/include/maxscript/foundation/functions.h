...
/*! Class that uses the RAII idiom to push/pop a CallContext. 
	This ensures that the CallContext is properly popped in the event of
	an exception. 
*/
class ScopedPushCallContext : public MaxSDK::Util::Noncopyable 
{
	CallContext* m_save_cc;
	bool m_CallContextPopped;
public:
	ScripterExport ScopedPushCallContext(CallContext* c);
	ScripterExport ~ScopedPushCallContext();
	ScripterExport void PopCallContext();
};
...
public:
	Struct* this_struct;
	Value*	fn;
	StructDef* owning_def;

	ScripterExport StructMethod(Struct* t, Value* f, StructDef* d = nullptr);
...
private:
	void	resolve_proxy();
	void	set_interface(FPInterface *pFPI);
