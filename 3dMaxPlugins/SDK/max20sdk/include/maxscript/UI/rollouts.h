...
#	define		is_rollout(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(Rollout))
	...
	ScripterExport BOOL add_page(Interface *pIp, HINSTANCE hInst, int ro_flags = 0, RolloutFloater* pRof = NULL);
	ScripterExport void delete_page(Interface *pIp, RolloutFloater* pRof = NULL);
	void		open(Interface *pIp, BOOL rolled_up = FALSE);
	ScripterExport void close(Interface *pIp, RolloutFloater* pRof = NULL);
	ScripterExport BOOL ok_to_close(RolloutFloater* pRof = NULL);
...
public:
	...
	HWND			window = 0;					// modeless dialog window

	HWND			ru_window = 0;				// host rollup window cust control
