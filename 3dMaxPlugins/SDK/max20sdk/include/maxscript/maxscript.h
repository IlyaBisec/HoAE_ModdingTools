...
ScripterExport void	    printable_name(MSTR& name);  // assumes name is localized, returns english name if name found in local_to_english
ScripterExport void	    printable_internal_name(MSTR& name);  // assumes name is not localized, no translation performed
...
/*! Class that uses the RAII idiom to push/pop the error traceback disable state. 
	...
	an exception. 
*/
class ScopedErrorTracebackDisable : public MaxSDK::Util::Noncopyable 
{
	MAXScript_TLS* m_tls;
...
/*! \param[in] filename - The fully qualified path to the existing file. This can be either
	...
	\param[in, out] error_message - String buffer for holding error message if any. Assumes size of string buffer supplied by caller is MAX_PATH.
...
namespace MAXScript
{
	//! \brief Sets MAXScript listener log file to specified file
	...
	//! \brief Gets the MAXScript listener log file name if present
