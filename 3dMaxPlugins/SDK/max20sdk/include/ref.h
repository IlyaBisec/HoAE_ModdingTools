...
/*! \note This message is for the most part no longer used. It is basically synonymous with 
	\ref REFMSG_SUBANIM_STRUCTURE_CHANGED. It is primarily used when changing the type of a light*/
...
/*! This message is sent by a node when it's TM has changed because it was evaluated
	...
	Note that when a node is being transformed in the viewport, 3ds Max starts a hold and 
	does a put, and then as the node is dragged the put is undone (to put the node back 
	to where it started from) and a new put occurs with the new transform. This is done 
	to prevent the node from walking away from the mouse cursor due to roundoff errors. 
	This message may occur within a undo entry that is undone as the node is moved. 
	Therefore performing scene operations in response to this message is typically not
	a good idea. */
...
//! \brief For internal use only. 
/*! Sent when updating object xrefs. When a node receives this message it will return REF_HALT if
it will set its material to the new one when handling REFMSG_OBJXREF_UPDATEMAT. */
#define REFMSG_OBJXREF_UPDATEMAT_QUERY	0x00000182
//! \brief For internal use only. 
/*! Sent when updating object xrefs. When a node receives this message it will return REF_HALT if
it will set its controller to the new one when handling REFMSG_OBJXREF_UPDATECTRL. */
...
#define TARGETMSG_USER				0x00010000
//! \brief Sent to a Node's Modifiers and ObjectRef when the node is attaching the object to itself	
#define TARGETMSG_ATTACHING_NODE	0x00000010		
//! \brief Sent to a Node's Modifiers and ObjectRef when the node is about to be explicitly deleted
#define TARGETMSG_DELETING_NODE		0x00000020		
//! \brief Sent to a Node's Modifiers and ObjectRef when the node is detaching the object from itself
#define TARGETMSG_DETACHING_NODE	0x00000030
//! \brief Sent to a Node's Modifiers and ObjectRef when the node's transform is changed.
...
//! \brief Specified as argument to NotifyDependents to specify level of optimizations used
enum NotifyDependentsOption {
	//! Duplicate messages (same hTarg, message, and partID) to same refmaker are blocked.
	//! Not all message types are blocked, some are always allowed as duplicates to allow proper operation of 3dsmax
	//! Note that this option can be turned off entirely by the user in the .ini by setting
	//! EnableOptimizeDependentNotifications=0
	REFNOTIFY_ALLOW_OPTIMIZATIONS = 0,
	//! Duplicate messages are not blocked
	REFNOTIFY_DISALLOW_OPTIMIZATIONS,
	//! Used internally when propagating notifications
    //
	// Optimizations permitted but not active; typically message is not optimizable (but may be
	// changed to an optimizable message further down the dependency chain)
	REFNOTIFY_INTERNAL_PROPAGATION_ALLOW_OPTIMIZATIONS,
	// Optimizations permitted and currently active
	REFNOTIFY_INTERNAL_PROPAGATION_ALLOW_OPTIMIZATIONS_ENABLED,
	// Optimizations not permitted
	REFNOTIFY_INTERNAL_PROPAGATION_DISALLOW_OPTIMIZATIONS,
};

//! \brief Returns true if this RefMessage can have redundant notifications optimized (inhibited).
//! \param  notifyDependentsOption - Specifies the level of optimization currently in use.
//! \param  message - Type of notification message that may be sent.
...
private:
	// We want the NotifyDependents method with the following signature to be sealed to force derived classes
	// that implement this method with this signature to modify their implementation to the new signature.
	//! \deprecated Deprecated method in terms of implementation as of 3ds Max 2017 - re-implement with 'notifyDependentsOption' method signature
		...
		{
			UNUSED_PARAM(changeInt); UNUSED_PARAM(partID);
			UNUSED_PARAM(message); UNUSED_PARAM(sclass);
			UNUSED_PARAM(propagate); UNUSED_PARAM(hTarg);
			DbgAssert(false);
			return REF_FAIL;
		};
public:
...
//@{	
		...
		*/
		...
		//! \brief Update class type tracking for this refmaker
		void UpdateClassTypeTracking();
...
//@{
		...
		\param notifyDependentsOption - This parameter specifies whether duplicate
			messages to the same refmaker can potentially be optimized away
...
{
	friend DependentIteratorHelper;
public:
	//! \brief Constructor
...
public:
	...
	CoreExport SingleRefMaker(AutoDropRefOnShutdown autoDropRefOnShutdown = Never); 

	//! \brief Constructor
	/*! \param refTarg - the reference to hold. */
	/*! \param autoDropRefOnShutdown - specifies whether/when to auto drop the Reference on shutdown. */
	CoreExport SingleRefMaker(ReferenceTarget* refTarg, AutoDropRefOnShutdown autoDropRefOnShutdown = Never);
	...
	//! \brief Deletes this SingleRefMaker, marshalling the deletion to the main 3ds Max thread if needed.
	/*! Animatable-derived class instances should always be created and deleted in the main thread, or
	exclusively in a worker thread. 3ds Max uses several managed classes that use a SingleRefMaker to
	hold a reference to a scene object, and those managed classes can be deleted from the .net garbage
	collection finalizer thread. This method is provided to safely delete this SingleRefMaker instance.
	If called from the main 3ds Max thread, the instance is immediately deleted. If called from another
	thread, the instance is scheduled for deletion by the main thread. 
	*/
...
template < class T>
class TypedSingleRefMaker : public SingleRefMaker
{
public:
	//! \brief ctors
	TypedSingleRefMaker(AutoDropRefOnShutdown autoDropRefOnShutdown = Never) : SingleRefMaker(autoDropRefOnShutdown) {}
	TypedSingleRefMaker(T* refTarg, AutoDropRefOnShutdown autoDropRefOnShutdown = Never) : SingleRefMaker(static_cast<T*>(refTarg), autoDropRefOnShutdown) {}
...
public:
	...
	*/
	SuspendAll(
		...
		BOOL refMsgs = FALSE,
		BOOL sceneExplorerUpdates = FALSE);
