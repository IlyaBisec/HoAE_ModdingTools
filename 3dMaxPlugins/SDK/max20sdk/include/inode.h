...
class INode: public ReferenceTarget, public FPMixinInterface {
	public:
		...
		// Prevents methods not overridden by this class to be hidden by the overridden version
...
//! @{		
        ...
        */
        ...
        \par Example
        \snippet samples/mesh/EditPoly/EditPolyData.cpp INode.GetActualINode Example 
...
//! @{		
        ...
        \par Example
        \snippet howto/characterstudio/Crowd/Formation/formation.cpp INode.GetNodeTM Example
        ...
        */
        ...
        \par Example
        \snippet howto/ui/customcontrols/custctrl.cpp INode.SetNodeTM Example
...
//! \name Invalidate the Node's Caches
//! @{		
...
//! @}
        ...
        \par Example
        \snippet howto/manipulators/bend/bendmanip.cpp INode.InvalidateRect Example        
...
//! @{		
		...
		<b>GetObjTMAfterWSM()</b> or <b>GetObjTMBeforeWSM()</b>.
		Developers should use <b>GetObjTMBeforeWSM()</b> if what is
		...
		*/
		...
		~{ Geometry Pipeline System }~ for additional details on this method.
		...
		*/
		...
		~{ Geometry Pipeline System }~ for additional details on this method. Note: Beware
...
//! \name Parent-Child Hierarchy Manipulation 
//! @{		
        ...
        \par Example
        \snippet samples/controllers/lodctrl.cpp INode.GetParentNode Example
        */
...
//! @{		
		...
		//! \brief Set the X-Ray node property
...
//! @{
		...
		/*! \remarks  Returns non-zero if the renderable flag is on; zero if off. */
...
//! @{		
		...
		\param onOff Non-zero for on; zero for off. */
		virtual void    SetCVertMode(int onOff)		{}
		
		/*! \remarks Returns non-zero if the vertex color flag if on; otherwise zero. */
		...
		\param onOff Non-zero of on; zero of off. */
		virtual void    SetShadeCVerts(int onOff)	{}
		
		/*! \remarks Returns non-zero if the vertex color shaded flag is on; zero if off. */
		virtual int     GetShadeCVerts()			{return 0;}
		
		/*! \remarks Get the node's vertex color "type".  This is one of the 
		nodeVertexColorType enums above. */
...
//! \name Display Attributes
//! @{		
		/*! \remarks Returns non-zero if the trajectory display is on; zero if the trajectory display is off. */
		...
		\param onOff   Non-zero if bones should be shown as lines only; 0 for normal display. */
...
//! @{		
        ...
        \par Example
        \snippet samples/objects/camera.cpp INode.SetTargetNodePair Example
		\param onOff TRUE for on; FALSE for off.
		\par Default Implementation:
		<b>{}</b> */
...
//! @{	
		...
		\return Non-zero if the node is selected; otherwise 0. */
		virtual int 	Selected()=0;
		
		/*! \remarks Returns non-zero if the node has its dependent flag
...
//! @{		
		...
		have a parent node).
        \par Example
        \snippet howto/utilities/xref/xrefutil.cpp INode.IsRootNode() Example        
		\return    Non-zero if the node is the root node; otherwise 0. */
...
//! @{	
		...
		\return    Non-zero if the node is a target; otherwise 0. */
...
//! @{		
		...
		~{ Geometry Pipeline System }~     section for additional details. */
...
//! @{			
		...
		\endcode
        \par Example
        \snippet howto/utilities/transformmatrixviz/tmattest.cpp INode.GetTMController Example
...
//! \name Visibility Related Methods
//! @{		
        ...
        \par Example
        \snippet howto/render/cjrender/instance.cpp INode.GetVisibility Example
		...
		\param valid The validity interval to update based on the validity of the visibility. 
		\return The visibility of the node as rendered. Values <= 0 are invisible, and
...
//! @{		
        ...
        \par Example
        \snippet howto/render/cjrender/instance.cpp INode.GetMtl Example
        */
...
//! \name Put Nodes into the Foreground Plane
//! @{		
		// Misc.
...
//! \name Temporary Storage of Data with Nodes
//! @{
...
//! @{		
		...
		\param rd The render data to set.  */
		virtual void SetRenderData(RenderData *rd)=0;
//! @}

//! \name Access User Defined Property Text
//! @{
		//
...
//! \name Geometry/Graphics (G) Buffer ID Access
//! @{		
		...
		~{ Working with Bitmaps }~(G-Buffer) for additional details. */
...
//! @{		
		...
		on ~{ Transformation and Rotation }~ . */
...
//! @{		
		...
		<b>{return 0;}</b> 
        \par Example
        \snippet howto/utilities/xref/xrefutil.cpp INode.GetXRefFileCount Example
        */
		...
		<b>{return FALSE;}</b> 
        \par Example
        \snippet howto/utilities/xref/xrefutil.cpp INode.AddNewXRefFile Example
        */
        ...
        */
		...
		called when loading a new file, resetting or clearing the scene.
		\par Default Implementation:
		<b>{}</b> */
		...
		<b>{return FALSE;}</b> 
        \par Example
        \snippet howto/utilities/xref/xrefutil.cpp INode.ReloadXRef Example
        */
		...
		/*! \remarks This method tells if the INode is a scene XRef node or not.
		\return true if the node is a scene XRef node, false otherwise.
		*/
		virtual bool IsSceneXRefNode() const { return false; }
//! @}
...
//! @}
		...
		//! \name Node Was Loaded By The Last Merge
//! @{		
		/*! \remarks Determines if this node was loaded during last merge
		\return    true if the node was loaded during last merge; otherwise false. */
		virtual bool WasLoadedByLastMerge() const { return false; }
//! @}
...
#define XREF_SCENE_MERGE_DUPLICATEMATCHINGLAYERS		(1<<14)	//!< duplicate matching Layers without an identical parent hierarchy when merging.
//@}						
...
//
/*! \sa  Class INode, ~{ Modifier Stack Branching }~
\par Description:
...
class INodeTransformed : public INode {
	public:
		...
		const MCHAR* 	GetName() {return node->GetName();}
		void	SetName(const MCHAR *s) {node->SetName(s);}
		Matrix3	GetNodeTM(TimeValue t, Interval* valid=NULL) {return node->GetNodeTM(t,valid);}
		void 	SetNodeTM(TimeValue t, Matrix3& m) {node->SetNodeTM(t,m);}
		...
		void 	AttachChild(INode* n, int keepTM=1) {node->AttachChild(n,keepTM);}
		...
		void	SetPrimaryVisibility(bool onOff) override { node->SetPrimaryVisibility(onOff); }
		bool    GetPrimaryVisibility() const override { return node->GetPrimaryVisibility(); }
		void	SetSecondaryVisibility(bool onOff) override { node->SetSecondaryVisibility(onOff); }
		bool    GetSecondaryVisibility() const override { return node->GetSecondaryVisibility(); }
		...
		virtual void GetClassName(MSTR& s) { s = _M("INodeTransformed"); } // from Animatable
