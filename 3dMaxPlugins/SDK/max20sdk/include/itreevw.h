...
public:
	...
	//!\brief Sets up the Retimer RMB quad menu for setting the span type
	//!\ param name Quad Menu name
	...
	enum { tv_getHWnd, tv_getMenuBar, tv_setMenuBar, tv_getControllerQuadMenu, tv_setControllerQuadMenu,
			...
			tv_showToolbar, tv_hideToolbar, tv_getToolbarName, tv_getToolbarObjectName, tv_deleteToolbar, tv_addToolbar, tv_toolbarCount,
	...
	public:
		...
		*/
		...
		enum {
			...
			tv_setFilter, tv_clearFilter, tv_testFilter,
			...
			tv_autoZoomOnSelChange, tv_setAutoZoomOnSelChange,
			tv_reduceKeys,
			tv_splitKey,
			tv_stepPreviousKeySelection, tv_stepNextKeySelection,
			tv_shrinkKeySelection, tv_growKeySelection,
			tv_easeToPreviousKey, tv_easeToNextKey,
			tv_mirrorKeyTangents,
			tv_nudgeKey,
			tv_scaleTangentsDown, tv_scaleTangentsUp,
			tv_loopKey,
			tv_snapScaleOriginsToFirstSelectedKey,
			tv_spaceKeyEvenly,
			tv_relaxKeys,
			tv_flattenKeys, tv_averageKeys,
		...
		BEGIN_FUNCTION_MAP
			...
			VFN_1(tv_relaxKeys, RelaxKeys, TYPE_BOOL)
			VFN_1(tv_flattenKeys, FlattenKeys, TYPE_BOOL)
			VFN_1(tv_averageKeys, AverageKeys, TYPE_BOOL)
...
//	private:
		...
		//! \brief function-mapping version of SelectTrackByIndex, throwing an exception on an invalid index
		...
		//! \brief Remove redundant keys of selected tracks (or just the selected keys if keys are selected for a track)
		//! \param[in] in_threshold if removing a key results in a change of value less than the tolerance, the key is removed.
		virtual void ReduceKeys(float in_threshold) = 0;

		//! \brief Deletes all the selected keys and creates two new keys for each deleted key, one on the previous frame, the other one on the next frame.
		virtual void SplitKey() = 0;
		//! \brief Moves the key selection backwards.
		virtual void StepPreviousKeySelection() = 0;
		//! \brief Moves the key selection forward.
		virtual void StepNextKeySelection() = 0;
		//! \brief Shrinks key selection.
		virtual void ShrinkKeySelection() = 0;
		//! \brief Grows Key Selection.
		virtual void GrowKeySelection() = 0;
		//! \brief Eases the selected key value to the previous key.
		virtual void EaseToPreviousKey() = 0;
		//! \brief Eases the selected key value to the next key.
		virtual void EaseToNextKey() = 0;
		//! \brief  Mirrors the tangents of the selected key to the adjacent ones.
		virtual void MirrorKeyTangents() = 0;
		//! \brief Nudges key selection (moves the selected keys on the next frame).
		//! \param[in] toLeft When true, nudges the key selection to the previous frame.
		virtual void NudgeKey(BOOL toLeft) = 0;
		//! \brief Scales tangents length and angle down.
		//! \param[in] onlyInnerHandle When true, scales only the inner tangent handles of the selection.
		virtual void ScaleTangentsDown(BOOL onlyInnerHandle) = 0;
		//! \brief Scales tangents length and angle up.
		//! \param[in] onlyInnerHandle When true, scales only the inner tangent handles of the selection.
		virtual void ScaleTangentsUp(BOOL onlyInnerHandle) = 0;
		//! \brief Copies the first key value to the last frame of the current animation range.
		//! \param[in] toLastKey When true, copies the first key value of the current animation to the last one.
		virtual void LoopKey(BOOL toLastKey) = 0;
		//! \brief Works with Scales Values tool.Moves the scale origin to the first selected key.
		virtual void SnapScaleOriginsToFirstSelectedKey() = 0;
		//! \brief Spaces the key selection evenly in time between the first and last keys
		//! \param[in] inValue When true, spaces evenly in value.
		virtual void SpaceKeyEvenly(BOOL inValue) = 0;
		//! \brief Eases key values and tangents between the first and last keys of the selection.
		//! \param[in] toLin When true, aligns the keys between the first and last keys of the selection
		virtual void RelaxKeys(BOOL toLin) = 0;
		//! \brief Flattens the selected keys to the first selected key value.
		//! \param[in] toLeft When true, flattens the selected keys to the value of the last selected one.
		virtual void FlattenKeys(BOOL toLeft) = 0;
		//! \brief Flattens the selected keys to the average value of the selected keys.
		//! \param[in] weld When true, welds the selected keys.
...
class ITreeView : public IObject, public ITreeViewOps{
	public:
		...
		*/
		...
		//from IObject
		virtual const MCHAR* GetIObjectName(){return _M("ITrackView");}
...
protected:
	...
	public:	
		...
		*/
		...
		/*! \remarks This method sets a track view layout as the default,  
		the one that will be used by the LaunchFCurveEditor command
		//! \param[in] layoutName  the name of the layout to be used as default
		!*/
		virtual void SetDefaultCurveEditorLayout(const MCHAR* layoutName);
		/*! \remarks This method returns the name of the default curve editor layout. */
		...
		enum{ getTrackView, getAvaliableTrackViews, getNumAvailableTrackViews,
			  ...
			  setFilter, clearFilter, testFilter, pickTrackDlg, isOpen, openLastTrackView, currentTrackViewProp, currentTrackViewProp2,
			  lastUsedTrackViewNameProp, deleteTrackView, isTrackViewCurrent, setTrackViewCurrent,
			  doesTrackViewExist, 

			  dockTypeEnum,
			  getDefaultCurveEditorLayout, setDefaultCurveEditorLayout,
			};
		...
		PROP_FNS(getDefaultCurveEditorLayout, GetDefaultCurveEditorLayout, 
			setDefaultCurveEditorLayout, SetDefaultCurveEditorLayout, TYPE_STRING);
		END_FUNCTION_MAP
		#pragma warning(pop)
	private:
		...
		BOOL fpTestTrackViewFilter(FPParams* val);
		...
		BOOL fpOpenTrackViewWindow(FPValue* val,MSTR layoutName, Point2 pos, int width, int height, int dock, bool applyUIScaling);
...
class TrackSelectionSetMgr : public FPStaticInterface
	{
public:
	virtual BOOL	Create(MSTR name, Tab<ReferenceTarget*> tracks, const Tab<const MCHAR*> &trackNames)=0;
	...
	virtual void	GetTracks(MSTR name, Tab<ReferenceTarget*> &tracks, Tab<const MCHAR*> &trackNames)=0;
	virtual BOOL	Add(MSTR name, Tab<ReferenceTarget*> tracks, const Tab<const MCHAR*> &trackNames)=0;
...
#define ID_TV_RETIMER_TOOL		489 //!< Activate/Deactivate the Retimer Tool mode
...
#define ID_TV_RETIMER_LINEAR	531	//!< Retimer span types
...
#define ID_TV_XREF_USE_XREFED 850
#define ID_TV_XREF_USE_LOCAL 851

#define ID_TV_PUBLISH_PARAMETERS	860
...
#define ID_TV_STATSTIMETYPEIN	1071
#define ID_TV_VALUETYPEIN		1080
#define ID_TV_STATSVALUETYPEIN	1081
#define ID_TV_ZOOM				1090
#define ID_TV_MAXIMIZE			1100
#define ID_TV_SELWILDCARD		1110
#define ID_TV_ZOOMSEL			1120
#define ID_TV_HV_FITTOWINDOW	1130
#define ID_TV_TOGGLE_AUTOZOOM	1131
...
#define ID_TV_SHOWBUFFER_CURVES		2402
#define ID_TV_SWAP_WITH_BUFFER		2403
#define ID_TV_SNAPSHOT_TO_BUFFER	2404
#define ID_TV_REVERT_TO_BUFFER		2405
#define ID_TV_USEBUFFER_CURVES		2406

#define ID_TV_SNAP_SCALE               2410
#define ID_TV_ISOLATE                  2411
#define ID_TV_BUILD_UP_TANGENTS        2412
#define ID_TV_BUILD_DOWN_TANGENTS      2413
#define ID_TV_FLATTEN                  2414
#define ID_TV_GROW_SELECTION           2415
#define ID_TV_RELAX_KEY				   2416
#define ID_TV_STEP_SELECTION           2419
#define ID_TV_FLOW                     2422
#define ID_TV_NUDGE                    2423
#define ID_TV_SPLIT                    2424
#define ID_TV_BREAKDOWN_NEXT           2425
#define ID_TV_AVERAGE				   2427
#define ID_TV_LOOP_KEY				   2428
#define ID_TV_SPACE_KEY_EVENLY		   2429
