...
public:
	...
	*/
	...
	/*! \brief Checks if a file or directory exists in the location pointed to by the absolute path parameter, considering IMissingPathCache.
	...
	*/
	...
	\param vflags If FILE_ENUM_MISSING_ONLY is set, only paths that cannot
...
To access the single IPathConfigMgrEx2 instance, call the static function
IPathConfigMgrEx2::GetPathConfigMgr().
*/
class IPathConfigMgrEx2 : public IPathConfigMgrEx
{
public:

	/*! \brief Checks if a file or directory exists in the location pointed to by the absolute path parameter, potentially considering the missing file cache.
	...
	\return true if the path is absolute and points to a file or directory that currently exists
	\sa Class IMissingPathCache
	*/
	virtual bool DoesFileExist(const MaxSDK::Util::Path& aPath, bool ignoreMissingPathCache) const = 0;
	...
	\return The single instance of this class.
	*/
	CoreExport static IPathConfigMgrEx2* GetPathConfigMgr();
};
