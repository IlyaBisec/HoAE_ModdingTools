...
TCHAR* VertexProcessingTypeToString(VertexProcessingType vpt)
{
    switch (vpt)
    {
        case vpSoftware:        return TEXT("Software VP");
        case vpMixed:           return TEXT("Mixed VP");
        case vpHardware:        return TEXT("Hardware VP");
        case vpPureHardware:    return TEXT("Pure Hardware VP");
...
//-----------------------------------------------------------------------------
void CD3DSettingsDialog::ComboBoxAdd( int id, const void* pData, LPCTSTR pstrDesc )
{
    ...
    case WM_INITDIALOG:
        {
            ...
            for( UINT i = 0; i < m_pEnumeration->m_AdapterInfoList.size(); i++ )
            {
                const D3DAdapterInfo& adInfo = m_pEnumeration->m_AdapterInfoList[i];
                TCHAR strDescription[512];
                DXUtil_ConvertAnsiStringToGenericCch( strDescription, adInfo.m_Identifier.Description, 512 );
                ComboBoxAdd( IDC_ADAPTER_COMBO, &adInfo, strDescription );
                if (adInfo.m_Ordinal == m_d3dSettings.Ordinal()) ComboBoxSelect( IDC_ADAPTER_COMBO, &adInfo );
            }
...
void CD3DSettingsDialog::AdapterChanged( void )
{
    ...
    if( m_d3dSettings.m_bIsWindowed )
    {
        ...
        {
            D3DDISPLAYMODE Mode;
            if( SUCCEEDED( pD3D->GetAdapterDisplayMode( pAdapterInfo->m_Ordinal, &Mode ) ) )
                m_d3dSettings.Windowed_DisplayMode.Format = Mode.Format;
            SAFE_RELEASE( pD3D );
        }
    }
    else m_d3dSettings.pFullscreen_AdapterInfo = pAdapterInfo;

    // Update device combo box
    ComboBoxClear( IDC_DEVICE_COMBO );
    for (UINT i = 0; i < pAdapterInfo->m_DevInfos.size(); i++)
    {
        const D3DDeviceInfo& devInfo = pAdapterInfo->m_DevInfos[i];
        ComboBoxAdd( IDC_DEVICE_COMBO, &devInfo, D3DDevTypeToString( devInfo.m_DevType ) );
        if (devInfo.m_DevType == m_d3dSettings.DevType()) 
        {
...
} // CD3DSettingsDialog::AdapterChanged
...
void CD3DSettingsDialog::DeviceChanged( void )
{
    ...
    if (m_d3dSettings.m_bIsWindowed && HasWindowedDeviceCombo)
    {
    ...
    WindowedFullscreenChanged();
} // CD3DSettingsDialog::DeviceChanged
...
void CD3DSettingsDialog::WindowedFullscreenChanged( void )
{
    D3DAdapterInfo* pAdapterInfo = (D3DAdapterInfo*)ComboBoxSelected( IDC_ADAPTER_COMBO );
    D3DDeviceInfo*  pDeviceInfo  = (D3DDeviceInfo*)ComboBoxSelected( IDC_DEVICE_COMBO );
    if( pAdapterInfo == NULL || pDeviceInfo == NULL ) return;
    ...
    else
    {
        ...
        for (int i = 0; i < pDeviceInfo->m_DevCombos.size(); i++ )
        {
            ...
            D3DFORMAT adapterFormat = devCombo.m_AdapterFormat;
...
void CD3DSettingsDialog::AdapterFormatChanged( void )
{
    if( !IsDlgButtonChecked( m_hDlg, IDC_WINDOW ) )
    {
        ...
        ComboBoxClear( IDC_RESOLUTION_COMBO );
        for( UINT idm = 0; idm < pAdapterInfo->m_DisplayModes.size(); idm++ )
        {
            D3DDISPLAYMODE displayMode = pAdapterInfo->m_DisplayModes[idm];
    ...
    for (UINT idc = 0; idc < pDeviceInfo->m_DevCombos.size(); idc++)
    {
        const D3DDeviceCombo& devCombo = pDeviceInfo->m_DevCombos[idc];
        if (devCombo.m_bIsWindowed == m_d3dSettings.m_bIsWindowed &&
            devCombo.m_AdapterFormat == m_d3dSettings.DisplayMode().Format)
        {
            if (!ComboBoxContainsText( IDC_BACKBUFFERFORMAT_COMBO, 
                D3DUtil_D3DFormatToString( devCombo.m_BackBufferFormat ) ) )
            {
                ComboBoxAdd( IDC_BACKBUFFERFORMAT_COMBO, (void*)devCombo.m_BackBufferFormat,
                                D3DUtil_D3DFormatToString( devCombo.m_BackBufferFormat ) );
                if (devCombo.m_BackBufferFormat == m_d3dSettings.BackBufferFormat() )
                {
                    ComboBoxSelect( IDC_BACKBUFFERFORMAT_COMBO, (void*)devCombo.m_BackBufferFormat );
                }
            }
        }

        // Count the number of backbuffer format supported for windowed mode
        if( true == devCombo.m_bIsWindowed &&
            devCombo.m_AdapterFormat == m_d3dSettings.Windowed_DisplayMode.Format )
        {
...
} // CD3DSettingsDialog::AdapterFormatChanged
...
void CD3DSettingsDialog::BackBufferFormatChanged( void )
{
    ...
    {
        D3DDeviceCombo& devCombo = pDeviceInfo->m_DevCombos[i];
        if (devCombo.m_bIsWindowed == m_d3dSettings.m_bIsWindowed &&
            devCombo.m_AdapterFormat == adapterFormat &&
            devCombo.m_BackBufferFormat == backBufferFormat)
        {
            if( m_d3dSettings.m_bIsWindowed ) m_d3dSettings.pWindowed_DeviceCombo = &devCombo;
            else m_d3dSettings.pFullscreen_DeviceCombo = &devCombo;
            ComboBoxClear( IDC_DEPTHSTENCILBUFFERFORMAT_COMBO );
            if (m_pEnumeration->m_bUseZBuffer)
            {
                for( UINT ifmt = 0; ifmt < devCombo.m_DepthStencilFormats.size(); ifmt++ )
                {
                    D3DFORMAT fmt = *(D3DFORMAT*)devCombo.m_DepthStencilFormats[ifmt];
                    ComboBoxAdd( IDC_DEPTHSTENCILBUFFERFORMAT_COMBO, (void*)fmt, D3DUtil_D3DFormatToString(fmt) );
            ...
            for( UINT ivpt = 0; ivpt < devCombo.m_VProcessTypes.size(); ivpt++ )
            {
                VertexProcessingType vpt = devCombo.m_VProcessTypes[ivpt];
                ComboBoxAdd( IDC_VERTEXPROCESSING_COMBO, (void*)vpt, VertexProcessingTypeToString(vpt) );
                if (vpt == m_d3dSettings.GetVertexProcessingType()) ComboBoxSelect( IDC_VERTEXPROCESSING_COMBO, (void*)vpt );
            }
            ...
            for( UINT ipi = 0; ipi < devCombo.m_PresentIntervals.size(); ipi++ )
            {
                UINT pi = devCombo.m_PresentIntervals[ipi];
...
} // CD3DSettingsDialog::BackBufferFormatChanged

//-----------------------------------------------------------------------------
// Name: DSBufferFormatChanged
// Desc: Respond to a change of selected depth/stencil buffer format.
//-----------------------------------------------------------------------------
void CD3DSettingsDialog::DSBufferFormatChanged( void )
{
    D3DFORMAT fmt = (D3DFORMAT)PtrToUlong( ComboBoxSelected( IDC_DEPTHSTENCILBUFFERFORMAT_COMBO ) );
    if (m_pEnumeration->m_bUseZBuffer) m_d3dSettings.SetDepthStencilBufferFormat( fmt );
    ...
    for( UINT ims = 0; ims < pDeviceCombo->m_MSampleTypes.size(); ims++ )
    {
        ...
        for( UINT iConf = 0; iConf < pDeviceCombo->m_DSMSConflicts.size(); iConf++ )
        {
            D3DDSMSConflict* pDSMSConf = &pDeviceCombo->m_DSMSConflicts[iConf];
            if( pDSMSConf->m_DSFormat == fmt && pDSMSConf->m_MSType == msType )
            {
                bConflictFound = TRUE;
                break;
            }
...
} // CD3DSettingsDialog::DSBufferFormatChanged
...
void CD3DSettingsDialog::MultisampleTypeChanged( void )
{
    ...
    for (int ims = 0; ims < pDeviceCombo->m_MSampleTypes.size(); ims++)
    {
        ...
        if( msType == mst )
        {
            maxQuality = pDeviceCombo->m_MSampleQualities[ims];
            break;
        }
    }
...
} // CD3DSettingsDialog::MultisampleTypeChanged
...
} // CD3DSettingsDialog::MultisampleQualityChanged
...
} // CD3DSettingsDialog::VertexProcessingChanged
...
} // CD3DSettingsDialog::PresentIntervalChanged
...
} // CD3DSettingsDialog::DeviceClipChanged
