...
public:
    ...
    D3DXMATRIX*  GetViewTM()            { return &m_mView; }
    D3DXMATRIX*  GetProjTM()            { return &m_mProj; }
...
public:
    ...
    // Functions to get state
    D3DXMATRIX* GetWorldTM() { return &m_mWorld; }
...
#define SAFE_RELEASE(A)     { if(A) {	int __nRef = (A)->Release(); \
                                (A)=NULL; \
                                if ( __nRef != 0 ) Log.Error( "Invalid D3D object refcount: %d", __nRef ); } }
#define SAFE_DECREF(A)			{ if(A) { (A)->Release(); } }
#define FORCE_RELEASE(A)	{ if(A) {	while((A)->Release()); (A)=NULL; } }

inline int GetRefCount( IUnknown* pObj )
{
    pObj->AddRef();
    int nRef = 1;
    while (pObj->Release() > 1) nRef++;
    for (int i = 1; i < nRef; i++) pObj->AddRef();
    return nRef;
} // GetRefCount
